...
subprocess.run(
    ["cargo", "build", "--release", "-p", "nexuschat-rpc-server", "--target", target],
    check=True,
)
...
my_binary_name = "nexuschat-rpc-server"

if not path.isfile(binary_path("nexuschat-rpc-server")):
    my_binary_name = "nexuschat-rpc-server.exe"
    if not path.isfile(binary_path("nexuschat-rpc-server.exe")):
