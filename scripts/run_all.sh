#!/bin/sh
#
# Build the Nexus Chat Core Rust library and Python wheels
...
cargo build --release -p nexuschat_ffi

# Statically link against libnexuschat.a.
...
rm -rf src/nexuschat/__pycache__
mkdir -p $TOXWORKDIR
...
auditwheel repair "$TOXWORKDIR"/wheelhouse/nexuschat* -w "$TOXWORKDIR/wheelhouse"
