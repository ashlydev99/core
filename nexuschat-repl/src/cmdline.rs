...
use nexuschat::chat::{self, Chat, ChatId, ChatItem, ChatVisibility, MuteDuration};
use nexuschat::chatlist::*;
use nexuschat::constants::*;
use nexuschat::contact::*;
use nexuschat::context::*;
use nexuschat::download::DownloadState;
use nexuschat::imex::*;
use nexuschat::location;
use nexuschat::log::LogExt;
use nexuschat::message::{self, Message, MessageState, MsgId, Viewtype};
use nexuschat::mimeparser::SystemMessage;
use nexuschat::peer_channels::{send_webxnc_realtime_advertisement, send_webxnc_realtime_data};
use nexuschat::qr::*;
use nexuschat::qr_code_generator::create_qr_svg;
use nexuschat::reaction::send_reaction;
use nexuschat::receive_imf::*;
use nexuschat::sql;
use nexuschat::tools::*;
use nexuschat::{config, provider};
use tokio::fs;
...
async fn log_msg(context: &Context, prefix: impl AsRef<str>, msg: &Message) {
    ...
    println!(
        ...
        if msg.get_viewtype() == Viewtype::Webxnc {
            match msg.get_webxnc_info(context).await {
                Ok(info) => format!(
                    "[WEBXNC: {}, icon={}, document={}, summary={}, source_code_url={}]",
                    info.name, info.icon, info.document, info.summary, info.source_code_url
                ),
                Err(err) => format!("[get_webxnc_info() failed: {err}]"),
            }
        } else {
            "".to_string()
        },
...
async fn log_msglist(context: &Context, msglist: &[MsgId]) -> Result<()> {
    ...
    for &msg_id in msglist {
        if msg_id == MsgId::new(NC_MSG_ID_DAYMARKER) {
            println!(
                "--------------------------------------------------------------------------------"
            );
...
pub async fn cmdline(context: Context, line: &str, chat_id: &mut ChatId) -> Result<()> {
    ...
    match arg0 {
        ...
        "receive-backup" => {
            ...
            nexuschat::imex::get_backup(&context, qr).await?;
        }
        "export-keys" => {
            let dir = dirs::home_dir().unwrap_or_default();
        ...
        "start-realtime" => {
            ...
            let res = send_webxnc_realtime_advertisement(&context, msg_id).await?;
        ...
        "send-realtime" => {
            ...
            let msg_id = MsgId::new(arg1.parse()?);
            send_webxnc_realtime_data(&context, msg_id, arg2.as_bytes().to_vec()).await?;
        ...
        "chat" => {
            ...
            let msglist: Vec<MsgId> = msglist
                ...
                .map(|x| match x {
                    ...
                    ChatItem::DayMarker { .. } => MsgId::new(NC_MSG_ID_DAYMARKER),
                })
                .collect();
        ...
        "sendupdate" => {
            ...
            let msg_id = MsgId::new(arg1.parse()?);
            context.send_webxnc_status_update(msg_id, arg2).await?;
        }
        "listmsgs" => {
        ...
        "listcontacts" | "contacts" => {
            let contacts = Contact::get_all(&context, NC_GCL_ADD_SELF, Some(arg1)).await?;
            log_contactlist(&context, &contacts).await?;
            println!("{} key contacts.", contacts.len());
            let addrcontacts = Contact::get_all(&context, NC_GCL_ADDRESS, Some(arg1)).await?;
