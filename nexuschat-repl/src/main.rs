...
//! included when using Nexus Chat Core as a library.
//!
//! Usage:  cargo run --example repl --release -- <databasefile>
//! All further options can be set using the set-command (type ? for help).

extern crate nexuschat;

use std::borrow::Cow::{self, Borrowed, Owned};

use anyhow::{bail, Error};
use nexuschat::chat::ChatId;
use nexuschat::config;
use nexuschat::context::*;
use nexuschat::oauth2::*;
use nexuschat::qr_code_generator::get_securejoin_qr_svg;
use nexuschat::securejoin::*;
use nexuschat::EventType;
...
impl Helper for NcHelper {}
impl Validator for NcHelper {}
...
async fn start(args: Vec<String>) -> Result<(), Error> {
    ...
    let input_loop = tokio::task::spawn_blocking(move || {
        ...
        if rl.load_history(".nc-history.txt").is_err() {
        ...
        rl.save_history(".nc-history.txt")?;
        ...
        "oauth2" => {
            if let Some(addr) = ctx.get_config(config::Config::Addr).await? {
                if let Some(oauth2_url) =
                    get_oauth2_url(&ctx, &addr, "chat.nexus:/com.b44t.messenger").await?
                {
...
async fn main() -> Result<(), Error> {
    tracing_subscriber::fmt()
        .with_env_filter(
            EnvFilter::from_default_env().add_directive("nexuschat_repl=info".parse()?),
        )
        .init();
