...
clone the `chatmail core <https://github.com/ashlydev99/core/>`_ repository::

   git clone https://github.com/ashlydev99/core.git
...
to build debug version of ``nexuschat-rpc-server``
and run ``nexuschat-rpc-client`` tests
...
::
    ...
    pytest nexuschat-rpc-client/tests/

Refer to `pytest documentation <https://docs.pytest.org/>` for details.

If make the changes to Nexus Chat core
...
you can quickly test things in REPL::
   ...
   >>> rpc.start()
   >>> nc = NexusChat(rpc)
   >>> system_info = nc.get_system_info()
   >>> system_info["level"]
   'awesome'
   >>> rpc.close()
