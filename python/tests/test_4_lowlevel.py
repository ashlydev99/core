...
from nexuschat import capi, cutil, register_global_plugin
from nexuschat.capi import ffi, lib
from nexuschat.hookspec import global_hookimpl
from nexuschat.testplugin import (
    ACSetup,
    create_dict_from_files_in_path,
    write_dict_to_dir,
)
from nexuschat.cutil import from_optional_nc_charpointer

# from nexuschat.account import EventLogger
...
def test_empty_context():
    ctx = capi.lib.nc_context_new(capi.ffi.NULL, capi.ffi.NULL, capi.ffi.NULL)
    capi.lib.nc_context_unref(ctx)


def test_nc_close_events(acfactory):
    ac1 = acfactory.get_unconfigured_account()
    ...
    class ShutdownPlugin:
        @global_hookimpl
        def nc_account_after_shutdown(self, account):
            assert account._nc_context is None
            shutdowns.put(account)

    register_global_plugin(ShutdownPlugin())
    assert hasattr(ac1, "_nc_context")
    ac1.shutdown()
    shutdowns.get()
...
def test_wrong_db(tmp_path):
    ...
    context = lib.nc_context_new(ffi.NULL, str(p).encode("ascii"), ffi.NULL)
    assert not lib.nc_context_is_open(context)
...
def test_empty_blobdir(tmp_path):
    ...
    ctx = ffi.gc(
        lib.nc_context_new(ffi.NULL, str(db_fname).encode("ascii"), b""),
...
def test_event_defines():
    assert nc.const.NC_EVENT_INFO == 100
    assert nc.const.NC_CONTACT_ID_SELF


def test_sig():
    sig = capi.lib.nc_event_has_string_data
    assert not sig(nc.const.NC_EVENT_MSGS_CHANGED)
    assert sig(nc.const.NC_EVENT_INFO)
    assert sig(nc.const.NC_EVENT_WARNING)
    assert sig(nc.const.NC_EVENT_ERROR)
    assert sig(nc.const.NC_EVENT_SMTP_CONNECTED)
    assert sig(nc.const.NC_EVENT_IMAP_CONNECTED)
    assert sig(nc.const.NC_EVENT_SMTP_MESSAGE_SENT)
    assert sig(nc.const.NC_EVENT_IMEX_FILE_WRITTEN)
...
def test_markseen_invalid_message_ids(acfactory):
    ...
    ac1._evtracker.get_matching("NC_EVENT_MSGS_CHANGED")
    # Skip configuration-related warnings, but not errors.
    ac1._evtracker.ensure_event_not_queued("NC_EVENT_ERROR")
    msg_ids = [9]
    lib.nc_markseen_msgs(ac1._nc_context, msg_ids, len(msg_ids))
    ac1._evtracker.ensure_event_not_queued("NC_EVENT_WARNING|NC_EVENT_ERROR")
...
def test_provider_info_none():
    ctx = ffi.gc(
        lib.nc_context_new(ffi.NULL, ffi.NULL, ffi.NULL),
        lib.nc_context_unref,
    )
    assert lib.nc_provider_new_from_email(ctx, cutil.as_nc_charpointer("email@unexistent.no")) == ffi.NULL


def test_get_info_open(tmp_path):
    db_fname = tmp_path / "test.db"
    ctx = ffi.gc(
        lib.nc_context_new(ffi.NULL, str(db_fname).encode("ascii"), ffi.NULL),
        lib.nc_context_unref,
    )
    info = cutil.from_nc_charpointer(lib.nc_get_info(ctx))
    assert "nexuschat_core_version" in info
...
def test_logged_ac_process_ffi_failure(acfactory):
    from nexuschat import account_hookimpl
...
def test_jsonrpc_blocking_call(tmp_path):
    ...
    writable = True
    accounts = ffi.gc(
        lib.nc_accounts_new(str(accounts_fname).encode("ascii"), writable),
        lib.nc_accounts_unref,
    )
    jsonrpc = ffi.gc(lib.nc_jsonrpc_init(accounts), lib.nc_jsonrpc_unref)
    res = json.loads(
        from_optional_nc_charpointer(
            lib.nc_jsonrpc_blocking_call(
                jsonrpc,
                json.dumps(
