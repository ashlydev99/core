...
def test_qr_verified_group_and_chatting(acfactory, lp):
    ...
    assert ac2.get_self_contact().get_verifier(ac2_ac1_contact).id == nc.const.NC_CONTACT_ID_SELF
    ...
    for ac2_contact in chat2.get_contacts():
        if ac2_contact == ac2_ac1_contact or ac2_contact.id == nc.const.NC_CONTACT_ID_SELF:
            continue
...
def test_ephemeral_timer(acfactory, lp):
    ...
    ac2._evtracker.get_matching("NC_EVENT_CHAT_EPHEMERAL_TIMER_MODIFIED")
    ...
    ac1._evtracker.get_matching("NC_EVENT_CHAT_EPHEMERAL_TIMER_MODIFIED")
...
def test_verified_group_vs_delete_server_after(acfactory, tmp_path, lp):
    ...
    ev = ac2_offl._evtracker.get_matching("NC_EVENT_INCOMING_MSG|NC_EVENT_MSGS_CHANGED")
...
def test_deleted_msgs_dont_reappear(acfactory):
    ...
    ac1._evtracker.get_matching("NC_EVENT_MSG_DELETED")
    ac1._evtracker.get_matching("NC_EVENT_IMAP_MESSAGE_DELETED")
    time.sleep(5)
