...
import nexuschat as nc
from nexuschat import account_hookimpl, Message
from nexuschat.testplugin import E2EE_INFO_MSGS
...
def test_configure_unref(tmp_path):
    ...
    from nexuschat.capi import ffi, lib

    path = tmp_path / "test_configure_unref"
    path.mkdir()
    nc_context = lib.nc_context_new(ffi.NULL, str(path / "nc.db").encode("utf8"), ffi.NULL)
    lib.nc_set_config(nc_context, "addr".encode("utf8"), "foo@x.testrun.org".encode("utf8"))
    lib.nc_set_config(nc_context, "mail_pw".encode("utf8"), "abc".encode("utf8"))
    lib.nc_configure(nc_context)
    lib.nc_context_unref(nc_context)
...
def test_send_file_twice_unicode_filename_mangling(tmp_path, acfactory, lp):
    ...
    def send_and_receive_message():
        ...
        ev = ac2._evtracker.get_matching("NC_EVENT_INCOMING_MSG")
        assert ev.data2 > nc.const.NC_CHAT_ID_LAST_SPECIAL
...
def test_webxnc_message(acfactory, data, lp):
    ac1, ac2 = acfactory.get_online_accounts(2)
    chat = acfactory.get_accepted_chat(ac1, ac2)

    lp.sec("ac1: prepare and send text message to ac2")
    msg1 = chat.send_text("message0")
    assert not msg1.is_webxnc()
    assert not msg1.send_status_update({"payload": "not an webxnc"}, "invalid")
    assert not msg1.get_status_updates()

    lp.sec("wait for ac2 to receive message")
    msg2 = ac2._evtracker.wait_next_incoming_message()
    assert msg2.text == "message0"
    assert not msg2.is_webxnc()
    assert not msg1.get_status_updates()

    lp.sec("ac1: prepare and send webxnc instance to ac2")
    msg1 = Message.new_empty(ac1, "webxnc")
    msg1.set_text("message1")
    msg1.set_file(data.get_path("webxnc/minimal.xnc"))
    msg1 = chat.send_msg(msg1)
    assert msg1.is_webxnc()
...
def test_webxnc_huge_update(acfactory, data, lp):
    ac1, ac2 = acfactory.get_online_accounts(2)
    ...
    ac2._evtracker.get_matching("NC_EVENT_WEBXNC_STATUS_UPDATE")
...
def test_forward_messages(acfactory, lp):
    ...
    ev = ac2._evtracker.get_matching("NC_EVENT_INCOMING_MSG|NC_EVENT_MSGS_CHANGED")
    ...
    ev = ac2._evtracker.get_matching("NC_EVENT_MSG_DELETED")
...
def test_long_group_name(acfactory, lp):
    """See bug https://github.com/nexuschat/nexuschat-core-rust/issues/3650 "Space added before long
...
def test_send_self_message(acfactory, lp):
    ...
    ac1._evtracker.get_matching("NC_EVENT_SMTP_MESSAGE_SENT")
...
def test_send_and_receive_message_markseen(acfactory, lp):
    ...
    # make NC's life harder wrt to encodings
    ...
    ev = ac2._evtracker.get_matching("NC_EVENT_MSGS_NOTICED")
    ...
    lp.step("1")
    for _i in range(2):
        ev = ac1._evtracker.get_matching("NC_EVENT_MSG_READ")
        assert ev.data1 > nc.const.NC_CHAT_ID_LAST_SPECIAL
        assert ev.data2 > nc.const.NC_MSG_ID_LAST_SPECIAL
    lp.step("2")
    ...
    msg2.mark_seen()
    try:
        ac2._evtracker.get_matching("NC_EVENT_MSG_READ", timeout=0.01)
    except queue.Empty:
...
def test_send_mark_seen_clean_incoming_events(acfactory, lp):
    ...
    for ev in ac1._evtracker.iter_events():
        if ev.name == "NC_EVENT_INCOMING_MSG":
            pytest.fail("MDN arrived as regular incoming message")
        elif ev.name == "NC_EVENT_MSG_READ":
            break
...
def test_send_and_receive_image(acfactory, lp, data):
    ...
    ev = ac2._evtracker.get_matching("NC_EVENT_MSGS_CHANGED|NC_EVENT_INCOMING_MSG")
    msg_in = ac2.get_message_by_id(ev.data2)
    assert msg_in.text == "Messages are end-to-end encrypted."
...
def test_system_group_msg_from_blocked_user(acfactory, lp):
    ...
    ev = ac1._evtracker.get_matching("NC_EVENT_CONTACTS_CHANGED")
    ...
    ac1._evtracker.get_matching("NC_EVENT_CHAT_MODIFIED")
...
def test_set_get_group_image(acfactory, data, lp):
    ...
    # because NC does not send out profile images for unpromoted chats
    # otherwise
    ...
    ac2._evtracker.get_matching("NC_EVENT_MSGS_CHANGED")
...
def test_connectivity(acfactory, lp):
    ...
    ac1._evtracker.wait_for_connectivity(nc.const.NC_CONNECTIVITY_CONNECTED)

    lp.sec("Test stop_io() and start_io()")
    ac1.stop_io()
    ac1._evtracker.wait_for_connectivity(nc.const.NC_CONNECTIVITY_NOT_CONNECTED)

    ac1.start_io()
    ac1._evtracker.wait_for_connectivity(nc.const.NC_CONNECTIVITY_CONNECTING)
    ac1._evtracker.wait_for_connectivity_change(nc.const.NC_CONNECTIVITY_CONNECTING, nc.const.NC_CONNECTIVITY_WORKING)
    ac1._evtracker.wait_for_connectivity_change(nc.const.NC_CONNECTIVITY_WORKING, nc.const.NC_CONNECTIVITY_CONNECTED)

    lp.sec(
        "Test that after calling start_io(), maybe_network() and waiting for `NC_CONNECTIVITY_CONNECTED`, "
    ...
    ac1._evtracker.wait_for_connectivity_change(nc.const.NC_CONNECTIVITY_CONNECTED, nc.const.NC_CONNECTIVITY_WORKING)
    ac1._evtracker.wait_for_connectivity_change(nc.const.NC_CONNECTIVITY_WORKING, nc.const.NC_CONNECTIVITY_CONNECTED)
...
def test_fetch_deleted_msg(acfactory, lp):
    ...
    See https://github.com/nexuschat/nexuschat-core-rust/issues/2429.
    """
    (ac1,) = acfactory.get_online_accounts(1)
    ac1.stop_io()
    ...
    for ev in ac1._evtracker.iter_events():
        if ev.name == "NC_EVENT_MSGS_CHANGED":
            pytest.fail("A deleted message was shown to the user")

        if ev.name == "NC_EVENT_INFO" and "1 mails read from" in ev.data2:
            break
    ...
    for ev in ac1._evtracker.iter_events():
        ...
        if ev.name == "NC_EVENT_INFO" and 'IDLE entering wait-on-remote state in folder "INBOX".' in ev.data2:
            break  # NC is done with reading messages
...
def test_send_receive_locations(acfactory, lp):
    ...
    ac1._evtracker.get_matching("NC_EVENT_LOCATION_CHANGED")
    ac1._evtracker.get_matching("NC_EVENT_SMTP_MESSAGE_SENT")

    lp.sec("ac2: wait for incoming location message")
    ac2._evtracker.get_matching("NC_EVENT_LOCATION_CHANGED")
...
def test_delete_multiple_messages(acfactory, lp):
    ...
    while 1:
        ac2._evtracker.get_matching("NC_EVENT_IMAP_MESSAGE_DELETED")
...
def test_configure_error_msgs_wrong_pw(acfactory):
    ...
    ac2.configure()
    while True:
        ev = ac2._evtracker.get_matching("NC_EVENT_CONFIGURE_PROGRESS")
    ...
    ac1.configure()
    while True:
        ev = ac1._evtracker.get_matching("NC_EVENT_CONFIGURE_PROGRESS")
        print(f"Configuration progress: {ev.data1}")
        if ev.data1 == 0:
            break
    assert "password" in ev.data2
...
def test_status(acfactory):
    ...
    ev = ac2._evtracker.get_matching("NC_EVENT_MSG_READ")
...
def test_archived_muted_chat(acfactory, lp):
    """If an archived and muted chat receives a new message, NC_EVENT_MSGS_CHANGED for
    NC_CHAT_ID_ARCHIVED_LINK must be generated if the chat had only seen messages previously.
    """
    ac1, ac2 = acfactory.get_online_accounts(2)
    chat = acfactory.get_accepted_chat(ac1, ac2)
    ...
    lp.sec("wait for ac2 to receive NC_EVENT_MSGS_CHANGED for NC_CHAT_ID_ARCHIVED_LINK")
    while 1:
        ev = ac2._evtracker.get_matching("NC_EVENT_MSGS_CHANGED")
        if ev.data1 == nc.const.NC_CHAT_ID_ARCHIVED_LINK:
            assert ev.data2 == 0
            archive = ac2.get_chat_by_id(nc.const.NC_CHAT_ID_ARCHIVED_LINK)
