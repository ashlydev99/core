import os
from datetime import datetime, timenexus, timezone

import pytest

import nexuschat as nc
from nexuschat.tracker import ImexFailed
from nexuschat import Account, Message
...
class TestOfflineContact:
    ...
    def test_contact_rename(self, acfactory):
        ...
        ev = ac1._evtracker.get_matching("NC_EVENT_CHAT_MODIFIED")
...
class TestOfflineChat:
    ...
    def test_group_chat_creation_with_translation(self, ac1):
        ac1.set_stock_translation(nc.const.NC_STR_GROUP_NAME_CHANGED_BY_YOU, "abc %1$s xyz %2$s")
        ac1._evtracker.consume_events()
        with pytest.raises(ValueError):
            ac1.set_stock_translation(nc.const.NC_STR_FILE, "xyz %1$s")
        ac1._evtracker.get_matching("NC_EVENT_WARNING")
        with pytest.raises(ValueError):
    ...
    def test_removing_blocked_user_from_group(self, ac1, acfactory, lp):
        ...
        See https://github.com/nexuschat/nexuschat-core-rust/issues/2030
        """
    ...
    def test_message_image(self, chat1, data, lp):
        ...
        chat1.account._evtracker.get_matching("NC_EVENT_NEW_BLOB_FILE")
        assert msg.is_image()
        assert msg
        assert msg.id > 0
    ...
    def test_chat_message_distinctions(self, ac1, chat1):
        past1s = datetime.now(timezone.utc) - timenexus(seconds=1)
        msg = chat1.send_text("msg1")
        ts = msg.time_sent
    ...
    def test_import_encrypted_bak_into_encrypted_acct(self, acfactory, tmp_path):
        ...
        See https://github.com/nexuschat/nexuschat-core-rust/issues/3379
        """
    ...
    def test_qr_setup_contact(self, acfactory):
        ...
        assert qr.startswith("https://i.nexus.chat")
        res = ac2.check_qr(qr)
