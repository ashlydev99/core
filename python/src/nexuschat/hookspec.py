"""Hooks for Python bindings to Nexus Chat Core Rust CFFI."""

import pluggy

account_spec_name = "nexuschat-account"
account_hookspec = pluggy.HookspecMarker(account_spec_name)
account_hookimpl = pluggy.HookimplMarker(account_spec_name)

global_spec_name = "nexuschat-global"
...
class PerAccount:
    ...
    def ac_process_ffi_event(self, ffi_event):
        ...
        with `NC_EVENT_* <https://c.nexus.chat/group__NC__EVENT.html>`_.
        """
...
class Global:
    ...
    @global_hookspec
    def nc_account_init(self, account):
        """called when `Account::__init__()` function starts executing."""

    @global_hookspec
    def nc_account_extra_configure(self, account):
