...
def pytest_addoption(parser):
    group = parser.getgroup("nexuschat testplugin options")
    group.addoption(
        "--chatmail",
        action="store",
        default=None,
...
def pytest_report_header(config):
    info = get_core_info()
    summary = [
        "Nexuschat core={} sqlite={} journal_mode={}".format(
            info["nexuschat_core_version"],
            info["sqlite_version"],
            info["journal_mode"],
        ),
    ]
    ...
    def init_imap(self, acc):
        ...
        if addr not in self._imap_cleaned:
            ...
            for folder in imap.list_folders():
                if folder.lower() == "inbox" or folder.lower() == "nexuschat":
...
class ACFactory:
    ...
    def _getaccount(self, try_cache_addr=None, closed=False) -> Account:
        ...
        path = self.tmpdir.mkdir(logid).join("nc.db")
        if try_cache_addr:
