...
def get_core_info():
    ...
    with NamedTemporaryFile() as path:
        path.close()
        return get_nc_info_as_dict(
            ffi.gc(
                lib.nc_context_new(as_nc_charpointer(""), as_nc_charpointer(path.name), ffi.NULL),
                lib.nc_context_unref,
            ),
        )


def get_nc_info_as_dict(nc_context):
    lines = from_nc_charpointer(lib.nc_get_info(nc_context))
    info_dict = {}
...
class Account:
    ...
    by the underlying nexuschat core library.  All public Account methods are
    ...
    def __init__(self, db_path, os_name=None, logging=True, closed=False) -> None:
        ...
        ptr = lib.nc_context_new_closed(db_path) if closed else lib.nc_context_new(ffi.NULL, db_path, ffi.NULL)
        if ptr == ffi.NULL:
            raise ValueError(f"Could not nc_context_new: {os_name} {db_path}")
        self._nc_context = ffi.gc(
            ptr,
            lib.nc_context_unref,
        )
        ...
        hook.nc_account_init(account=self)
    ...
    def open(self, passphrase: Optional[str] = None) -> bool:
        ...
        """
        return bool(lib.nc_context_open(self._nc_context, as_nc_charpointer(passphrase)))
    ...
    def get_info(self) -> Dict[str, str]:
        ...
        return get_nc_info_as_dict(self._nc_context)
    ...
    def set_stock_translation(self, id: int, string: str) -> None:
        ...
        :param id: id of stock string (const.NC_STR_*)
        :param value: string to set as new translation
        :returns: None
        """
        bytestring = string.encode("utf8")
        res = lib.nc_set_stock_translation(self._nc_context, id, bytestring)
        if res == 0:
    ...
    def set_config(self, name: str, value: Optional[str]) -> None:
        ...
        lib.nc_set_config(self._nc_context, namebytes, valuebytes)
    ...
    def get_config(self, name: str) -> str:
        ...
        if name != "sys.config_keys":
            self._check_config_key(name)
        namebytes = name.encode("utf8")
        res = lib.nc_get_config(self._nc_context, namebytes)
    ...
    def _preconfigure_keypair(self, secret: str) -> None:
        """See nc_preconfigure_keypair() in nexuschat.h.

        In other words, you don't need this.
        """
        res = lib.nc_preconfigure_keypair(
            self._nc_context,
            as_nc_charpointer(secret),
        )
        if res == 0:
    ...
    def is_configured(self) -> bool:
        ...
        """
        return bool(lib.nc_is_configured(self._nc_context))
    ...
    def is_open(self) -> bool:
        ...
        """
        return bool(lib.nc_context_is_open(self._nc_context))
    ...
    def get_latest_backupfile(self, backupdir) -> Optional[str]:
        ...
        res = lib.nc_imex_has_backup(self._nc_context, as_nc_charpointer(backupdir))
        return from_optional_nc_charpointer(res)
    ...
    def get_blobdir(self) -> str:
        ...
        """
        return from_nc_charpointer(lib.nc_get_blobdir(self._nc_context))

    def get_self_contact(self) -> Contact:
        """return this account's identity as a :class:`nexuschat.contact.Contact`.

        :returns: :class:`nexuschat.contact.Contact`
        """
        return Contact(self, const.NC_CONTACT_ID_SELF)
    ...
    def create_contact(self, obj, name: Optional[str] = None) -> Contact:
        ...
        :returns: :class:`nexuschat.contact.Contact` instance.
        """
        ...
        addr_c = as_nc_charpointer(addr)
        contact_id = lib.nc_create_contact(self._nc_context, name_c, addr_c)
    ...
    def delete_contact(self, contact: Contact) -> bool:
        ...
        assert contact_id > const.NC_CHAT_ID_LAST_SPECIAL
        return bool(lib.nc_delete_contact(self._nc_context, contact_id))
    ...
    def get_contact_by_addr(self, email: str) -> Optional[Contact]:
        ...
        **POTENTIAL SECURITY ISSUE**: If there are multiple contacts with this address
        ...
        addr = as_nc_charpointer(addr)
        contact_id = lib.nc_lookup_contact_id_by_addr(self._nc_context, addr)
        if contact_id:
    ...
    def get_blocked_contacts(self) -> List[Contact]:
        ...
        :returns: list of :class:`nexuschat.contact.Contact` objects.
        """
        nc_array = ffi.gc(lib.nc_get_blocked_contacts(self._nc_context), lib.nc_array_unref)
        return list(iter_array(nc_array, lambda x: Contact(self, x)))
    ...
    def get_contacts(
        ...
        """
        flags = 0
        query_c = as_nc_charpointer(query)
        if with_self:
            flags |= const.NC_GCL_ADD_SELF
        nc_array = ffi.gc(lib.nc_get_contacts(self._nc_context, flags, query_c), lib.nc_array_unref)
        return list(iter_array(nc_array, lambda x: Contact(self, x)))

    def import_vcard(self, vcard):
        """Import a vCard and return an array of contacts."""
        nc_array = ffi.gc(lib.nc_import_vcard(self._nc_context, as_nc_charpointer(vcard)), lib.nc_array_unref)
        return list(iter_array(nc_array, lambda x: Contact(self, x)))

    def get_fresh_messages(self) -> Generator[Message, None, None]:
        """yield all fresh messages from all chats."""
        nc_array = ffi.gc(lib.nc_get_fresh_msgs(self._nc_context), lib.nc_array_unref)
        return (x for x in iter_array(nc_array, lambda x: Message.from_db(self, x)) if x is not None)
    ...
    def create_group_chat(
        ...
        chat_id = lib.nc_create_group_chat(self._nc_context, 0, bytes_name)
    ...
    def get_chats(self) -> List[Chat]:
        ...
        :returns: a list of :class:`nexuschat.chat.Chat` objects.
        """
        nc_chatlist = ffi.gc(lib.nc_get_chatlist(self._nc_context, 0, ffi.NULL, 0), lib.nc_chatlist_unref)

        assert nc_chatlist != ffi.NULL
        chatlist = []
        for i in range(lib.nc_chatlist_get_cnt(nc_chatlist)):
            chat_id = lib.nc_chatlist_get_chat_id(nc_chatlist, i)
            chatlist.append(Chat(self, chat_id))
        return chatlist

    def get_device_chat(self) -> Chat:
        return Contact(self, const.NC_CONTACT_ID_DEVICE).create_chat()

    def get_message_by_id(self, msg_id: int) -> Optional[Message]:
        """return Message instance.
        :param msg_id: integer id of this message.
        :returns: :class:`nexuschat.message.Message` instance.
        """
    ...
    def get_chat_by_id(self, chat_id: int) -> Chat:
        ...
        :returns: :class:`nexuschat.chat.Chat` instance.
        :raises: ValueError if chat does not exist.
        """
        res = lib.nc_get_chat(self._nc_context, chat_id)
        if res == ffi.NULL:
            raise ValueError(f"cannot get chat with id={chat_id}")
        lib.nc_chat_unref(res)
    ...
    def mark_seen_messages(self, messages: List[Union[int, Message]]) -> None:
        ...
        lib.nc_markseen_msgs(self._nc_context, msg_ids, len(messages))

    def forward_messages(self, messages: List[Message], chat: Chat) -> None:
        """Forward list of messages to a chat.

        :param messages: list of :class:`nexuschat.message.Message` object.
        :param chat: :class:`nexuschat.chat.Chat` object.
        :returns: None
        """
        msg_ids = [msg.id for msg in messages]
        lib.nc_forward_msgs(self._nc_context, msg_ids, len(msg_ids), chat.id)
    ...
    def resend_messages(self, messages: List[Message]) -> None:
        ...
        """
        msg_ids = [msg.id for msg in messages]
        if lib.nc_resend_msgs(self._nc_context, msg_ids, len(msg_ids)) != 1:
    ...
    def delete_messages(self, messages: List[Message]) -> None:
        ...
        """
        msg_ids = [msg.id for msg in messages]
        lib.nc_delete_msgs(self._nc_context, msg_ids, len(msg_ids))
    ...
    def export_self_keys(self, path):
        ...
        """
        return self._export(path, imex_cmd=const.NC_IMEX_EXPORT_SELF_KEYS)
    ...
    def export_all(self, path: str, passphrase: Optional[str] = None) -> str:
        ...
        """
        export_files = self._export(path, const.NC_IMEX_EXPORT_BACKUP, passphrase)
    ...
    def import_self_keys(self, path):
        ...
        Note that the account does not have to be started.
        """
        self._import(path, imex_cmd=const.NC_IMEX_IMPORT_SELF_KEYS)

    def import_all(self, path: str, passphrase: Optional[str] = None) -> None:
        """Import Nexus Chat state from the specified backup file.
        ...
        self._import(path, imex_cmd=const.NC_IMEX_IMPORT_BACKUP, passphrase=passphrase)
    ...
    def imex(self, path: str, imex_cmd: int, passphrase: Optional[str] = None) -> None:
        lib.nc_imex(self._nc_context, imex_cmd, as_nc_charpointer(path), as_nc_charpointer(passphrase))

    def get_setup_contact_qr(self) -> str:
        """get/create Setup-Contact QR Code as ascii-string.

        this string needs to be transferred to another NC account
        in a second channel (typically used by mobiles with QRcode-show + scan UX)
        where qr_setup_contact(qr) is called.
        """
        res = lib.nc_get_securejoin_qr(self._nc_context, 0)
        return from_nc_charpointer(res)

    def check_qr(self, qr):
        """check qr code and return :class:`ScannedQRCode` instance representing the result."""
        res = ffi.gc(lib.nc_check_qr(self._nc_context, as_nc_charpointer(qr)), lib.nc_lot_unref)
        lot = NCLot(res)
        if lot.state() == const.NC_QR_ERROR:
    ...
    def qr_setup_contact(self, qr):
        ...
        chat_id = lib.nc_join_securejoin(self._nc_context, as_nc_charpointer(qr))
        if chat_id == 0:
    ...
    def set_location(self, latitude: float = 0.0, longitude: float = 0.0, accuracy: float = 0.0) -> None:
        ...
        :returns: None
        """
        nc_res = lib.nc_set_location(self._nc_context, latitude, longitude, accuracy)
        if nc_res == 0:
    ...
    def add_account_plugin(self, plugin, name=None):
        ...
        the :class:`nexuschat.hookspec.PerAccount` hooks.
        """
    ...
    def stop_ongoing(self):
        ...
        lib.nc_stop_ongoing_process(self._nc_context)

    def get_connectivity(self):
        return lib.nc_get_connectivity(self._nc_context)

    def get_connectivity_html(self) -> str:
        return from_nc_charpointer(lib.nc_get_connectivity_html(self._nc_context))
    ...
    def start_io(self):
        ...
        lib.nc_start_io(self._nc_context)
    ...
    def maybe_network(self):
        ...
        """
        lib.nc_maybe_network(self._nc_context)
    ...
    def configure(self) -> ConfigureTracker:
        ...
        lib.nc_configure(self._nc_context)
    ...
    def stop_io(self) -> None:
        ...
        self.log("nc_stop_io (stop core IO scheduler)")
        lib.nc_stop_io(self._nc_context)

    def shutdown(self) -> None:
        """shutdown and destroy account (stop callback thread, close and remove
        underlying nc_context).
        """
        if self._nc_context is None:
            return
        ...
        self.log("remove nc_context references, making the Account unusable")
        self._nc_context = None

        self._shutdown_event.set()

        hook = hookspec.Global._get_plugin_manager().hook
        hook.nc_account_after_shutdown(account=self)
...
class ScannedQRCode:
    ...
    def is_ask_verifycontact(self):
        return self._nc_lot.state() == const.NC_QR_ASK_VERIFYCONTACT

    def is_ask_verifygroup(self):
        return self._nc_lot.state() == const.NC_QR_ASK_VERIFYGROUP

    @property
    def contact_id(self):
        return self._nc_lot.id()
