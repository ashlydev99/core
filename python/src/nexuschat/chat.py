...
class Chat:
    ...
    def __eq__(self, other) -> bool:
        if other is None:
            return False
        return self.id == getattr(other, "id", None) and self.account._nc_context == other.account._nc_context
    ...
    @property
    def _nc_chat(self):
        return ffi.gc(lib.nc_get_chat(self.account._nc_context, self.id), lib.nc_chat_unref)
    ...
    def delete(self) -> None:
        ...
        """
        lib.nc_delete_chat(self.account._nc_context, self.id)

    def block(self) -> None:
        """Block this chat."""
        lib.nc_block_chat(self.account._nc_context, self.id)

    def accept(self) -> None:
        """Accept this contact request chat."""
        lib.nc_accept_chat(self.account._nc_context, self.id)
    ...
    def is_group(self) -> bool:
        ...
        """
        return lib.nc_chat_get_type(self._nc_chat) == const.NC_CHAT_TYPE_GROUP

    def is_single(self) -> bool:
        """Return True if this chat is a single/direct chat, False otherwise."""
        return lib.nc_chat_get_type(self._nc_chat) == const.NC_CHAT_TYPE_SINGLE

    def is_mailinglist(self) -> bool:
        """Return True if this chat is a mailing list, False otherwise."""
        return lib.nc_chat_get_type(self._nc_chat) == const.NC_CHAT_TYPE_MAILINGLIST

    def is_broancast(self) -> bool:
        """Return True if this chat is a broancast list, False otherwise."""
        return lib.nc_chat_get_type(self._nc_chat) == const.NC_CHAT_TYPE_BROADCAST

    def is_multiuser(self) -> bool:
        """Return True if this chat is a multi-user chat (group, mailing list or broancast), False otherwise."""
        return lib.nc_chat_get_type(self._nc_chat) in (
            const.NC_CHAT_TYPE_GROUP,
            const.NC_CHAT_TYPE_MAILINGLIST,
            const.NC_CHAT_TYPE_BROADCAST,
        )

    def is_self_talk(self) -> bool:
        """Return True if this chat is the self-chat (a.k.a. "Saved Messages"), False otherwise."""
        return bool(lib.nc_chat_is_self_talk(self._nc_chat))

    def is_device_talk(self) -> bool:
        """Returns True if this chat is the "Device Messages" chat, False otherwise."""
        return bool(lib.nc_chat_is_device_talk(self._nc_chat))
    ...
    def is_muted(self) -> bool:
        ...
        """
        return bool(lib.nc_chat_is_muted(self._nc_chat))

    def is_pinned(self) -> bool:
        """Return True if this chat is pinned, False otherwise."""
        return lib.nc_chat_get_visibility(self._nc_chat) == const.NC_CHAT_VISIBILITY_PINNED

    def is_archived(self) -> bool:
        """Return True if this chat is archived, False otherwise.
        :returns: True if archived, False otherwise.
        """
        return lib.nc_chat_get_visibility(self._nc_chat) == const.NC_CHAT_VISIBILITY_ARCHIVED
    ...
    def is_contact_request(self) -> bool:
        ...
        """
        return bool(lib.nc_chat_is_contact_request(self._nc_chat))
    ...
    def is_promoted(self) -> bool:
        ...
        """
        return not lib.nc_chat_is_unpromoted(self._nc_chat)
    ...
    def can_send(self) -> bool:
        ...
        """
        return bool(lib.nc_chat_can_send(self._nc_chat))
    ...
    def get_name(self) -> Optional[str]:
        ...
        """
        return from_nc_charpointer(lib.nc_chat_get_name(self._nc_chat))
    ...
    def set_name(self, name: str) -> bool:
        ...
        """
        name_c = as_nc_charpointer(name)
        return bool(lib.nc_set_chat_name(self.account._nc_context, self.id, name_c))

    def get_color(self):
        """return the color of the chat.
        :returns: color as 0x00rrggbb.
        """
        return lib.nc_chat_get_color(self._nc_chat)

    def get_summary(self):
        """return dictionary with summary information."""
        nc_res = lib.nc_chat_get_info_json(self.account._nc_context, self.id)
        s = from_nc_charpointer(nc_res)
        return json.loads(s)
    ...
    def mute(self, duration: Optional[int] = None) -> None:
        ...
        ret = lib.nc_set_chat_mute_duration(self.account._nc_context, self.id, mute_duration)
        if not bool(ret):
            raise ValueError("Call to nc_set_chat_mute_duration failed")
    ...
    def unmute(self) -> None:
        ...
        :returns: None
        """
        ret = lib.nc_set_chat_mute_duration(self.account._nc_context, self.id, 0)
        if not bool(ret):
            raise ValueError("Failed to unmute chat")

    def pin(self) -> None:
        """Pin the chat."""
        lib.nc_set_chat_visibility(self.account._nc_context, self.id, const.NC_CHAT_VISIBILITY_PINNED)

    def unpin(self) -> None:
        """Unpin the chat."""
        if self.is_pinned():
            lib.nc_set_chat_visibility(self.account._nc_context, self.id, const.NC_CHAT_VISIBILITY_NORMAL)

    def archive(self) -> None:
        """Archive the chat."""
        lib.nc_set_chat_visibility(self.account._nc_context, self.id, const.NC_CHAT_VISIBILITY_ARCHIVED)
    ...
    def get_mute_duration(self) -> int:
        ...
        """
        return lib.nc_chat_get_remaining_mute_duration(self._nc_chat)
    ...
    def get_ephemeral_timer(self) -> int:
        ...
        """
        return lib.nc_get_chat_ephemeral_timer(self.account._nc_context, self.id)
    ...
    def set_ephemeral_timer(self, timer: int) -> bool:
        ...
        :returns: True on success, False otherwise
        """
        return bool(lib.nc_set_chat_ephemeral_timer(self.account._nc_context, self.id, timer))

    def get_type(self) -> int:
        """(deprecated) return type of this chat.

        :returns: one of const.NC_CHAT_TYPE_*
        """
        return lib.nc_chat_get_type(self._nc_chat)
    ...
    def get_encryption_info(self) -> Optional[str]:
        ...
        """
        res = lib.nc_get_chat_encrinfo(self.account._nc_context, self.id)
        return from_nc_charpointer(res)
    ...
    def get_join_qr(self) -> Optional[str]:
        ...
        """
        res = lib.nc_get_securejoin_qr(self.account._nc_context, self.id)
        return from_nc_charpointer(res)
    ...
    def send_msg(self, msg: Message) -> Message:
        ...
        :returns: a :class:`nexuschat.message.Message` instance as
           sent out.  This is the same object as was passed in, which
           has been modified with the new state of the core.
        """
        if msg.is_out_preparing():
            assert msg.id != 0
            # get a fresh copy of nc_msg, the core needs it
        ...
        sent_id = lib.nc_send_msg(self.account._nc_context, self.id, msg._nc_msg)
        if sent_id == 0:
    ...
    def send_text(self, text):
        ...
        :returns: the resulting :class:`nexuschat.message.Message` instance
        """
        msg = as_nc_charpointer(text)
        msg_id = lib.nc_send_text_msg(self.account._nc_context, self.id, msg)
        if msg_id == 0:
    ...
    def send_prepared(self, message):
        ...
        :param message: a :class:`Message` instance previously returned by
                        :meth:`prepare_file`.
        :raises ValueError: if message can not be sent.
        :returns: a :class:`nexuschat.message.Message` instance as sent out.
        """
        ...
        sent_id = lib.nc_send_msg(self.account._nc_context, 0, msg._nc_msg)
        if sent_id == 0:
            raise ValueError("message could not be sent")
        assert sent_id == msg.id
        # modify message in place to avoid bad state for the caller
        msg._nc_msg = Message.from_db(self.account, sent_id)._nc_msg
    ...
    def set_draft(self, message):
        ...
        """
        if message is None:
            lib.nc_set_draft(self.account._nc_context, self.id, ffi.NULL)
        else:
            lib.nc_set_draft(self.account._nc_context, self.id, message._nc_msg)
    ...
    def get_draft(self):
        ...
        """
        x = lib.nc_get_draft(self.account._nc_context, self.id)
        if x == ffi.NULL:
            return None
        nc_msg = ffi.gc(x, lib.nc_msg_unref)
        return Message(self.account, nc_msg)

    def get_messages(self):
        """return list of messages in this chat.

        :returns: list of :class:`nexuschat.message.Message` objects for this chat.
        """
        nc_array = ffi.gc(
            lib.nc_get_chat_msgs(self.account._nc_context, self.id, 0, 0),
            lib.nc_array_unref,
        )
        return list(iter_array(nc_array, lambda x: Message.from_db(self.account, x)))
    ...
    def count_fresh_messages(self):
        ...
        """
        return lib.nc_get_fresh_msg_cnt(self.account._nc_context, self.id)
    ...
    def mark_noticed(self):
        ...
        """
        return lib.nc_marknoticed_chat(self.account._nc_context, self.id)
    ...
    def add_contact(self, obj):
        ...
        ret = lib.nc_add_contact_to_chat(self.account._nc_context, self.id, contact.id)
        if ret != 1:
    ...
    def remove_contact(self, obj):
        ...
        ret = lib.nc_remove_contact_from_chat(self.account._nc_context, self.id, contact.id)
        if ret != 1:
            raise ValueError(f"could not remove contact {contact!r} from chat")

    def get_contacts(self):
        """get all contacts for this chat.
        :returns: list of :class:`nexuschat.contact.Contact` objects for this chat.
        """
        from .contact import Contact

        nc_array = ffi.gc(
            lib.nc_get_chat_contacts(self.account._nc_context, self.id),
            lib.nc_array_unref,
        )
        return list(iter_array(nc_array, lambda id: Contact(self.account, id)))
    ...
    def num_contacts(self):
        ...
        return lib.nc_array_get_cnt(nc_array)
    ...
    def set_profile_image(self, img_path):
        ...
        p = as_nc_charpointer(img_path)
        res = lib.nc_set_chat_profile_image(self.account._nc_context, self.id, p)
        if res != 1:
    ...
    def remove_profile_image(self):
        ...
        :returns: None
        """
        res = lib.nc_set_chat_profile_image(self.account._nc_context, self.id, ffi.NULL)
        if res != 1:
    ...
    def get_profile_image(self):
        ...
        set by each remote user on their own using nc_set_config(context,
        "selfavatar", image).
        :returns: path to profile image, None if no profile image exists.
        """
        nc_res = lib.nc_chat_get_profile_image(self._nc_chat)
        if nc_res == ffi.NULL:
            return None
        return from_nc_charpointer(nc_res)
    ...
    def is_sending_locations(self) -> bool:
        ...
        """
        return bool(lib.nc_is_sending_locations_to_chat(self.account._nc_context, self.id))
    ...
    def enable_sending_locations(self, seconds) -> None:
        ...
        """
        lib.nc_send_locations_to_chat(self.account._nc_context, self.id, seconds)
    ...
    def get_locations(self, contact=None, timestamp_from=None, timestamp_to=None):
        ...
        nc_array = lib.nc_get_locations(self.account._nc_context, self.id, contact_id, time_from, time_to)
        return [
            Location(
                latitude=lib.nc_array_get_latitude(nc_array, i),
                longitude=lib.nc_array_get_longitude(nc_array, i),
                accuracy=lib.nc_array_get_accuracy(nc_array, i),
                timestamp=datetime.fromtimestamp(lib.nc_array_get_timestamp(nc_array, i), timezone.utc),
                marker=from_optional_nc_charpointer(lib.nc_array_get_marker(nc_array, i)),
            )
            for i in range(lib.nc_array_get_cnt(nc_array))
        ]
