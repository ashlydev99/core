...
class ImexTracker:
    ...
    def ac_process_ffi_event(self, ffi_event: "FFIEvent") -> None:
        if ffi_event.name == "NC_EVENT_IMEX_PROGRESS":
            self._imex_events.put(ffi_event.data1)
        elif ffi_event.name == "NC_EVENT_IMEX_FILE_WRITTEN":
...
class ConfigureTracker:
    ...
    def ac_process_ffi_event(self, ffi_event: "FFIEvent") -> None:
        ...
        if ffi_event.name == "NC_EVENT_SMTP_CONNECTED":
            self._smtp_finished.set()
        elif ffi_event.name == "NC_EVENT_IMAP_CONNECTED":
            self._imap_finished.set()
        elif ffi_event.name == "NC_EVENT_CONFIGURE_PROGRESS":
