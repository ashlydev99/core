...
from .cutil import as_nc_charpointer, from_nc_charpointer, from_optional_nc_charpointer


class Message:
    """Message object.

    You obtain instances of it through :class:`nexuschat.account.Account` or
    :class:`nexuschat.chat.Chat`.
    """

    def __init__(self, account, nc_msg) -> None:
        self.account = account
        assert isinstance(self.account._nc_context, ffi.CData)
        assert isinstance(nc_msg, ffi.CData)
        assert nc_msg != ffi.NULL
        self._nc_msg = nc_msg
        msg_id = self.id
    ...
    def from_db(cls, account, id) -> Optional["Message"]:
        ...
        assert id > 0
        res = lib.nc_get_msg(account._nc_context, id)
        if res == ffi.NULL:
            return None
        return cls(account, ffi.gc(res, lib.nc_msg_unref))
    ...
    def new_empty(cls, account, view_type):
        ...
        :param view_type: the message type code or one of the strings:
           "text", "audio", "video", "file", "sticker", "videochat", "webxnc"
        """
        view_type_code = view_type if isinstance(view_type, int) else get_viewtype_code_from_name(view_type)
        return Message(
            account,
            ffi.gc(lib.nc_msg_new(account._nc_context, view_type_code), lib.nc_msg_unref),
        )
    ...
    def id(self):
        """id of this message."""
        return lib.nc_msg_get_id(self._nc_msg)

    @props.with_doc
    def text(self) -> str:
        """unicode text of this messages (might be empty if not a text message)."""
        return from_nc_charpointer(lib.nc_msg_get_text(self._nc_msg))

    def set_text(self, text):
        """set text of this message."""
        lib.nc_msg_set_text(self._nc_msg, as_nc_charpointer(text))

    @props.with_doc
    def html(self) -> str:
        """html text of this messages (might be empty if not an html message)."""
        return from_optional_nc_charpointer(lib. c_get_msg_html(self.account._nc_context, self.id)) or ""

    def has_html(self):
        """return True if this message has an html part, False otherwise."""
        return lib.nc_msg_has_html(self._nc_msg)
    ...
    def set_html(self, html_text):
        ...
        """
        lib.nc_msg_set_html(self._nc_msg, as_nc_charpointer(html_text))
    ...
    def filename(self):
        ...
        return from_nc_charpointer(lib.nc_msg_get_file(self._nc_msg))

    def set_file(self, path, mime_type=None):
        """set file for this message from path and mime_type."""
        mtype = ffi.NULL if mime_type is None else as_nc_charpointer(mime_type)
        if not os.path.exists(path):
            raise ValueError(f"path does not exist: {path!r}")
        lib.nc_msg_set_file_and_deduplicate(self._nc_msg, as_nc_charpointer(path), ffi.NULL, mtype)
    ...
    def basename(self) -> str:
        ...
        return from_nc_charpointer(lib.nc_msg_get_filename(self._nc_msg))

    @props.with_doc
    def filemime(self) -> str:
        """mime type of the file (if it exists)."""
        return from_nc_charpointer(lib.nc_msg_get_filemime(self._nc_msg))

    def get_status_updates(self, serial: int = 0) -> list:
        """Get the status updates of this webxnc message.

        The status updates may be sent by yourself or by other members.
        If this message doesn't have a webxnc instance, an empty list is returned.

        :param serial: The last known serial. Pass 0 if there are no known serials to receive all updates.
        """
        return json.loads(
            from_nc_charpointer(lib. nc_get_webxnc_status_updates(self.account._nc_context, self.id, serial)),
        )

    def send_status_update(self, json_data: Union[str, dict], description: str) -> bool:
        """Send an status update for the webxnc instance of this message.
        ...
        return bool(
            lib.nc_send_webxnc_status_update(
                self.account._nc_context,
                self.id,
                as_nc_charpointer(json_data),
                as_nc_charpointer(description),
            ),
        )

    def is_system_message(self):
        """return True if this message is a system/info message."""
        return bool(lib.nc_msg_is_info(self._nc_msg))

    def is_encrypted(self):
        """return True if this message was encrypted."""
        return bool(lib.nc_msg_get_showpadlock(self._nc_msg))

    def is_bot(self):
        """return True if this message is submitted automatically."""
        return bool(lib.nc_msg_is_bot(self._nc_msg))

    def is_forwarded(self):
        """return True if this message was forwarded."""
        return bool(lib.nc_msg_is_forwarded(self._nc_msg))
    ...
    def get_message_info(self) -> str:
        ...
        """
        return from_nc_charpointer(lib.nc_get_msg_info(self.account._nc_context, self.id))

    def get_summarytext(self, width: int) -> str:
        """Get a message summary as a single line of text. Typically used for notifications."""
        return from_nc_charpointer(lib.nc_msg_get_summarytext(self._nc_msg, width))
    ...
    def time_sent(self):
        ...
        """
        ts = lib.nc_msg_get_timestamp(self._nc_msg)
    ...
    def time_received(self):
        ...
        """
        ts = lib.nc_msg_get_received_timestamp(self._nc_msg)
        if ts:
            return datetime.fromtimestamp(ts, timezone.utc)
        return None
    ...
    def ephemeral_timer(self):
        ...
        """
        timer = lib.nc_msg_get_ephemeral_timer(self._nc_msg)
        if timer:
            return timer
        return None
    ...
    def ephemeral_timestamp(self):
        ...
        """
        ts = lib.nc_msg_get_ephemeral_timestamp(self._nc_msg)
        if ts:
            return datetime.fromtimestamp(ts, timezone.utc)
    ...
    def quoted_text(self) -> Optional[str]:
        ...
        :returns: Quoted text
        """
        return from_optional_nc_charpointer(lib.nc_msg_get_quoted_text(self._nc_msg))
    ...
    def quote(self):
        ...
        """
        msg = lib.nc_msg_get_quoted_msg(self._nc_msg)
        if msg:
            return Message(self.account, ffi.gc(msg, lib.nc_msg_unref))

    @quote.setter
    def quote(self, quoted_message):
        """Quote setter."""
        lib.nc_msg_set_quote(self._nc_msg, quoted_message._nc_msg)

    @property
    def error(self) -> Optional[str]:
        """Error message."""
        return from_optional_nc_charpointer(lib.nc_msg_get_error(self._nc_msg))

    @property
    def chat(self):
        """chat this message was posted in.

        :returns: :class:`nexuschat.chat.Chat` object
        """
        from .chat import Chat

        chat_id = lib.nc_msg_get_chat_id(self._nc_msg)
    ...
    def override_sender_name(self) -> Optional[str]:
        ...
        """
        return from_optional_nc_charpointer(lib.nc_msg_get_override_sender_name(self._nc_msg))

    def set_override_sender_name(self, name):
        """set different sender name for a message."""
        lib.nc_msg_set_override_sender_name(self._nc_msg, as_nc_charpointer(name))
    ...
    def get_sender_contact(self):
        ...
        :returns: :class:`nexuschat.chat.Contact` instance
        """
        from .contact import Contact

        contact_id = lib.nc_msg_get_from_id(self._nc_msg)
        return Contact(self.account, contact_id)

    def is_from_self(self):
        """Return true if the message is sent by self."""
        contact_id = lib.nc_msg_get_from_id(self._nc_msg)
        return contact_id == const.NC_CONTACT_ID_SELF

    def is_from_device(self):
        """Return true if the message is sent by the device."""
        contact_id = lib.nc_msg_get_from_id(self._nc_msg)
        return contact_id == const.NC_CONTACT_ID_DEVICE
    ...
    def _msgstate(self):
        if self.id == 0:
            ...
            nc_msg = ffi.gc(lib.nc_get_msg(self.account._nc_context, self.id), lib.nc_msg_unref)
            # Message could be trashed, use the cached object if so.
            if nc_msg == ffi.NULL:
                nc_msg = self._nc_msg
        return lib.nc_msg_get_state(nc_msg)
    ...
    def is_in_fresh(self):
        ...
        """
        return self._msgstate == const.NC_STATE_IN_FRESH
    ...
    def is_in_noticed(self):
        ...
        """
        return self._msgstate == const.NC_STATE_IN_NOTICED
    ...
    def is_in_seen(self):
        ...
        are not counted as unread but were not marked as read nor resulted in MDNs.
        """
        return self._msgstate == const.NC_STATE_IN_SEEN

    def is_outgoing(self):
        """Return True if Message is outgoing."""
        return lib.nc_msg_get_state(self._nc_msg) in (
            const.NC_STATE_OUT_PREPARING,
            const.NC_STATE_OUT_PENDING,
            const.NC_STATE_OUT_FAILED,
            const.NC_STATE_OUT_MDN_RCVD,
            const.NC_STATE_OUT_DELIVERED,
        )

    def is_out_preparing(self):
        """Return True if Message is outgoing, but its file is being prepared."""
        return self._msgstate == const.NC_STATE_OUT_PREPARING

    def is_out_pending(self):
        """Return True if Message is outgoing, but is pending (no single checkmark)."""
        return self._msgstate == const.NC_STATE_OUT_PENDING

    def is_out_failed(self):
        """Return True if Message is unrecoverably failed."""
        return self._msgstate == const.NC_STATE_OUT_FAILED
    ...
    def is_out_delivered(self):
        ...
        """
        return self._msgstate == const.NC_STATE_OUT_DELIVERED
    ...
    def is_out_mdn_received(self):
        ...
        state, you'll receive the event NC_EVENT_MSG_READ.
        """
        return self._msgstate == const.NC_STATE_OUT_MDN_RCVD
    ...
    def _view_type(self):
        assert self.id > 0
        return lib.nc_msg_get_viewtype(self._nc_msg)

    def is_text(self):
        """return True if it's a text message."""
        return self._view_type == const.NC_MSG_TEXT

    def is_image(self):
        """return True if it's an image message."""
        return self._view_type == const.NC_MSG_IMAGE

    def is_gif(self):
        """return True if it's a gif message."""
        return self._view_type == const.NC_MSG_GIF

    def is_sticker(self):
        """return True if it's a sticker message."""
        return self._view_type == const.NC_MSG_STICKER

    def is_audio(self):
        """return True if it's an audio message."""
        return self._view_type == const.NC_MSG_AUDIO

    def is_video(self):
        """return True if it's a video message."""
        return self._view_type == const.NC_MSG_VIDEO

    def is_webxnc(self):
        """return True if it's a Webxnc message."""
        return self._view_type == const.NC_MSG_WEBXNC

    def is_file(self):
        """return True if it's a file message."""
        return self._view_type == const.NC_MSG_FILE
    ...
    def download_state(self):
        ...
        if nc_msg == ffi.NULL:
            nc_msg = self._nc_msg
        return lib.nc_msg_get_download_state(nc_msg)

    def download_full(self) -> None:
        lib.nc_download_full_msg(self.account._nc_context, self.id)


# some code for handling NC_MSG_* view types

_view_type_mapping = {
    "text": const.NC_MSG_TEXT,
    "image": const.NC_MSG_IMAGE,
    "gif": const.NC_MSG_GIF,
    "audio": const.NC_MSG_AUDIO,
    "video": const.NC_MSG_VIDEO,
    "file": const.NC_MSG_FILE,
    "sticker": const.NC_MSG_STICKER,
    "webxnc": const.NC_MSG_WEBXNC,
}
