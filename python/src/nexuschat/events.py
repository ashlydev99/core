...
from .cutil import from_optional_nc_charpointer
from .hookspec import account_hookimpl


def get_nc_event_name(integer, _NC_EVENTNAME_MAP={}):
    if not _NC_EVENTNAME_MAP:
        for name in dir(const):
            if name.startswith("NC_EVENT_"):
                _NC_EVENTNAME_MAP[getattr(const, name)] = name
    return _NC_EVENTNAME_MAP[integer]
...
class FFIEvent:
    ...
    def __str__(self) -> str:
        if self.name == "NC_EVENT_INFO":
            return f"INFO {self.data2}"
        if self.name == "NC_EVENT_WARNING":
            return f"WARNING {self.data2}"
        if self.name == "NC_EVENT_ERROR":
...
class FFIEventLogger:
    ...
    def ac_log_line(self, message):
        ...
        if os.name == "posix":
            ...
            ERROR = "\033[91m"
            ENNC = "\033[0m"
            if message.startswith("NC_EVENT_WARNING"):
                s = WARN + s + ENNC
            if message.startswith("NC_EVENT_ERROR"):
                s = ERROR + s + ENNC
        with self._loglock:
            print(s, flush=True)
...
class FFIEventTracker:
    ...
    def get(self, timeout=None, check_error=True):
        ...
        if check_error and ev.name == "NC_EVENT_ERROR":
    ...
    def get_info_contains(self, regex: str) -> FFIEvent:
        ...
        while True:
            ev = self.get_matching("NC_EVENT_INFO")
    ...
    def get_info_regex_groups(self, regex, check_error=True):
        rex = re.compile(regex)
        while True:
            ev = self.get_matching("NC_EVENT_INFO", check_error=check_error)
    ...
    def wait_for_connectivity(self, connectivity):
        ...
        while True:
            if self.account.get_connectivity() == connectivity:
                return
            self.get_matching("NC_EVENT_CONNECTIVITY_CHANGED")
    ...
    def wait_securejoin_inviter_progress(self, target):
        while True:
            event = self.get_matching("NC_EVENT_SECUREJOIN_INVITER_PROGRESS")
    ...
    def wait_securejoin_joiner_progress(self, target):
        while True:
            event = self.get_matching("NC_EVENT_SECUREJOIN_JOINER_PROGRESS")
            if event.data2 >= target:
                break
    ...
    def wait_idle_inbox_ready(self):
        ...
        - therefore no NC_EVENT_INCOMING_MSG is sent
        """
        self.get_matching("NC_EVENT_IMAP_INBOX_IDLE")

    def wait_next_incoming_message(self):
        """wait for and return next incoming message."""
        ev = self.get_matching("NC_EVENT_INCOMING_MSG")
    ...
    def wait_next_messages_changed(self):
        ...
        """
        ev = self.get_matching("NC_EVENT_MSGS_CHANGED")
        if ev.data2 > 0:
            return self.account.get_message_by_id(ev.data2)
        return None
    ...
    def wait_next_reactions_changed(self):
        ...
        ev = self.get_matching("NC_EVENT_REACTIONS_CHANGED")
        assert ev.data1 > 0
        return self.account.get_message_by_id(ev.data2)

    def wait_msg_delivered(self, msg):
        ev = self.get_matching("NC_EVENT_MSG_DELIVERED")
    ...
    def run(self) -> None:
        ...
        with self.log_execution("EVENT THREAD"):
            event_emitter = ffi.gc(
                lib.nc_get_event_emitter(self.account._nc_context),
                lib.nc_event_emitter_unref,
            )
            while not self._marked_for_shutdown:
                with self.swallow_and_log_exception("Unexpected error in event thread"):
                    event = lib.nc_get_next_event(event_emitter)
                    if event == ffi.NULL or self._marked_for_shutdown:
                        break
                    self._process_event(event)

    def _process_event(self, event) -> None:
        evt = lib.nc_event_get_id(event)
        data1 = lib.nc_event_get_data1_int(event)
        # the following code relates to the nexuschat/_build.py's helper
        # function which provides us signature info of an event call
        evt_name = get_nc_event_name(evt)
        if lib.nc_event_has_string_data(evt):
            data2 = from_optional_nc_charpointer(lib.nc_event_get_data2_str(event))
        else:
            data2 = lib.nc_event_get_data2_int(event)

        lib.nc_event_unref(event)
            ...
            if ffi_event.data2 != 0:
                ...
                if msg is not None:
                    if msg.is_outgoing():
                        ...
                        yield "ac_incoming_message", {"message": msg}
        elif name == "NC_EVENT_REACTIONS_CHANGED":
            assert ffi_event.data1 > 0
            msg = account.get_message_by_id(ffi_event.data2)
            if msg is not None:
                yield "ac_reactions_changed", {"message": msg}
        elif name == "NC_EVENT_MSG_DELIVERED":
            msg = account.get_message_by_id(ffi_event.data2)
            if msg is not None:
                yield "ac_message_delivered", {"message": msg}
        elif name == "NC_EVENT_CHAT_MODIFIED":
