...
use deltachat::key::{NcKey, SignedPublicKey};
use serde::Serialize;
...
pub struct ContactObject {
    ...
    /// If it is `NC_CONTACT_ID_SELF`, we verified the contact ourself.
...
impl ContactObject {
    pub async fn try_from_dc_contact(
        context: &Context,
        contact: nexuschat::contact::Contact,
    ) -> Result<Self> {
...
impl From<nexuschat_contact_tools::VcardContact> for VcardContact {
    fn from(vc: nexuschat_contact_tools::VcardContact) -> Self {
        ...
        let color = nexuschat::contact::get_color(is_self, &vc.addr, &fpr);
        Self {
            addr: vc.addr,
            display_name,
            key: vc.key,
