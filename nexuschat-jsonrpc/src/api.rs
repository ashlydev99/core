...
pub use nexuschat::accounts::Accounts;
use nexuschat::blob::BlobObject;
use nexuschat::calls::ice_servers;
use nexuschat::chat::{
...
use nexuschat::config::{get_all_ui_config_keys, Config};
use nexuschat::constants::NC_MSG_ID_DAYMARKER;
use nexuschat::contact::{may_be_valid_addr, Contact, ContactId, Origin};
use nexuschat::context::get_info;
use nexuschat::ephemeral::Timer;
use nexuschat::imex;
use nexuschat::location;
use nexuschat::message::{
...
use nexuschat::peer_channels::{
    leave_webxnc_realtime, send_webxnc_realtime_advertisement, send_webxnc_realtime_data,
};
use nexuschat::provider::get_provider_info;
use nexuschat::qr::{self, Qr};
use nexuschat::qr_code_generator::{create_qr_svg, generate_backup_qr, get_securejoin_qr_svg};
use nexuschat::reaction::{get_msg_reactions, send_reaction};
use nexuschat::securejoin;
use nexuschat::stock_str::StockMessage;
use nexuschat::storage_usage::{get_blobdir_storage_usage, get_storage_usage};
use nexuschat::webxnc::StatusUpdateSerial;
use nexuschat::EventEmitter;
...
use types::webxnc::WebxncMessageInfo;
...
impl CommandApi {
    ...
    async fn get_context_opt(&self, id: u32) -> Option<nexuschat::context::Context> {
        self.accounts.read().await.get_account(id)
    }

    async fn get_context(&self, id: u32) -> Result<nexuschat::context::Context> {
...
impl CommandApi {
    ...
    async fn get_provider_info(
        ...
        Ok(ProviderInfo::from_nc_type(provider_info))
    }
    ...
    ///
    /// "NCACCOUNT:" and "NCLOGIN:" QR codes configure the account, but I/O mustn't be started for
    ...
    async fn get_full_chat_by_id(&self, account_id: u32, chat_id: u32) -> Result<FullChat> {
        let ctx = self.get_context(account_id).await?;
        FullChat::try_from_nc_chat_id(&ctx, chat_id).await
    }
    ...
    async fn get_basic_chat_info(&self, account_id: u32, chat_id: u32) -> Result<BasicChat> {
        let ctx = self.get_context(account_id).await?;
        BasicChat::try_from_nc_chat_id(&ctx, chat_id).await
    }
    ...
    ///   group
    ///
    /// - for broancast channels, all recipients are returned, NC_CONTACT_ID_SELF is not included
    ///
    ...
    /// Deprecated 2025-07 in favor of create_broancast().
    async fn create_broancast_list(&self, account_id: u32) -> Result<u32> {
        self.create_broancast(account_id, "Channel".to_string())
            .await
    }

    /// Create a new, outgoing **broancast channel**
    /// (called "Channel" in the UI).
    ///
    /// Broancast channels are similar to groups on the sending device,
    ...
    async fn create_broancast(&self, account_id: u32, chat_name: String) -> Result<u32> {
        let ctx = self.get_context(account_id).await?;
        chat::create_broancast(&ctx, chat_name)
            .await
            .map(|id| id.to_u32())
    }
    ...
    /// Set group or broancast channel description.
    ///
    /// If the group is already _promoted_ (any message was sent to the group),
    /// or if this is a brodacast channel,
    /// all members are informed by a special status message that is sent automatically by this function.
    ...
    async fn add_device_message(
        ...
        if let Some(msg) = msg {
            ...
            let message_id =
                nexuschat::chat::add_device_msg(&ctx, Some(&label), Some(&mut message)).await?;
            if !message_id.is_unset() {
                return Ok(Some(message_id.to_u32()));
            }
        } else {
            nexuschat::chat::add_device_msg(&ctx, Some(&label), None).await?;
        }
        Ok(None)
    }
    ...
    ///
    /// * `add_daymarker` - If `true`, add day markers as `NC_MSG_ID_DAYMARKER` to the result,
    ...
    async fn get_message_ids(
        ...
        Ok(msg
            ...
            .map(|chat_item| -> u32 {
                match chat_item {
                    nexuschat::chat::ChatItem::Message { msg_id } => msg_id.to_u32(),
                    nexuschat::chat::ChatItem::DayMarker { .. } => NC_MSG_ID_DAYMARKER,
                }
            })
            .collect())
    }
    ...
    ///
    /// Global search results are typically displayed using nc_msg_get_summary(), chat
    ...
    async fn get_contact(&self, account_id: u32, contact_id: u32) -> Result<ContactObject> {
        ...
        ContactObject::try_from_nc_contact(
            &ctx,
            nexuschat::contact::Contact::get_by_id(&ctx, contact_id).await?,
        )
        .await
    }
    ...
    async fn get_blocked_contacts(&self, account_id: u32) -> Result<Vec<ContactObject>> {
        ...
        for id in blocked_ids {
            contacts.push(
                ContactObject::try_from_nc_contact(
                    &ctx,
                    nexuschat::contact::Contact::get_by_id(&ctx, id).await?,
                )
                .await?,
            );
        }
    ...
    ///   - `NC_GCL_ADD_SELF` - Add SELF unless filtered by other parameters.
    ...
    async fn get_contacts_by_ids(
        ...
        for id in ids {
            contacts.insert(
                ...
                ContactObject::try_from_nc_contact(
                    &ctx,
                    nexuschat::contact::Contact::get_by_id(&ctx, ContactId::new(id)).await?,
                )
                .await?,
            );
        }
    ...
    async fn parse_vcard(&self, path: String) -> Result<Vec<VcardContact>> {
        ...
        Ok(nexuschat_contact_tools::parse_vcard(vcard)
            .into_iter()
            .map(|c| c.into())
            .collect())
    }
    ...
    async fn import_vcard(&self, account_id: u32, path: String) -> Result<Vec<u32>> {
        ...
        let vcard = str::from_utf8(&vcard)?;
        Ok(nexuschat::contact::import_vcard(&ctx, vcard)
            .await?
            .into_iter()
            .map(|c| c.to_u32())
            .collect())
    ...
    async fn make_vcard(&self, account_id: u32, contacts: Vec<u32>) -> Result<String> {
        ...
        nexuschat::contact::make_vcard(&ctx, &contacts).await
    }
    ...
    async fn send_webxnc_status_update(
        &self,
        account_id: u32,
        instance_msg_id: u32,
        update_str: String,
        ...
        let ctx = self.get_context(account_id).await?;
        ctx.send_webxnc_status_update(MsgId::new(instance_msg_id), &update_str)
            .await
    }

    async fn send_webxnc_realtime_data(
        &self,
        account_id: u32,
        instance_msg_id: u32,
        data: Vec<u8>,
        ...
        let ctx = self.get_context(account_id).await?;
        send_webxnc_realtime_data(&ctx, MsgId::new(instance_msg_id), data).await
    }

    async fn send_webxnc_realtime_advertisement(
        &self,
        account_id: u32,
        instance_msg_id: u32,
    ) -> Result<()> {
        let ctx = self.get_context(account_id).await?;
        if let Some(fut) =
            send_webxnc_realtime_advertisement(&ctx, MsgId::new(instance_msg_id)).await?
        {
            tokio::spawn(fut);
        }
        Ok(())
    ...
    /// Leaves the gossip of the webxnc with the given message id.
    ///
    /// NB: When this is called before closing a webxnc app in UIs, it must be guaranteed that
    /// `send_webxnc_realtime_*()` functions aren't called for the given `instance_message_id`
    /// anymore until the app is open again.
    async fn leave_webxnc_realtime(&self, account_id: u32, instance_message_id: u32) -> Result<()> {
        let ctx = self.get_context(account_id).await?;
        leave_webxnc_realtime(&ctx, MsgId::new(instance_message_id)).await
    }

    async fn get_webxnc_status_updates(
        &self,
        account_id: u32,
        instance_msg_id: u32,
    ...
    /// Get info from a webxnc message
    async fn get_webxnc_info(
        &self,
        account_id: u32,
        instance_msg_id: u32,
    ) -> Result<WebxncMessageInfo> {
        let ctx = self.get_context(account_id).await?;
        WebxncMessageInfo::get_for_message(&ctx, MsgId::new(instance_msg_id)).await
    }

    /// Get href from a WebxncInfoMessage which might include a hash holding
    /// information about a specific position or state in a webxnc app (optional)
    async fn get_webxnc_href(&self, account_id: u32, info_msg_id: u32) -> Result<Option<String>> {
        let ctx = self.get_context(account_id).await?;
        let message = Message::load_from_db(&ctx, MsgId::new(info_msg_id)).await?;
        Ok(message.get_webxnc_href())
    }

    /// Get blob encoded as base64 from a webxnc message
    ///
    /// path is the path of the file within webxnc archive
    async fn get_webxnc_blob(
        &self,
        account_id: u32,
        instance_msg_id: u32,
        path: String,
        ...
        let blob = message.get_webxnc_blob(&ctx, &path).await?;
    ...
    async fn set_webxnc_integration(&self, account_id: u32, file_path: String) -> Result<()> {
        let ctx = self.get_context(account_id).await?;
        ctx.set_webxnc_integration(&file_path).await
    }
    ...
    async fn init_webxnc_integration(
        &self,
        account_id: u32,
        chat_id: Option<u32>,
    ) -> Result<Option<u32>> {
        let ctx = self.get_context(account_id).await?;
        Ok(ctx
            .init_webxnc_integration(chat_id.map(ChatId::new))
            .await?
            .map(|msg_id| msg_id.to_u32()))
    }
    ...
    async fn get_http_response(&self, account_id: u32, url: String) -> Result<HttpResponse> {
        let ctx = self.get_context(account_id).await?;
        let response = nexuschat::net::read_url_blob(&ctx, &url).await?.into();
        Ok(response)
    }
    ...
    async fn send_sticker(
        ...
        let message_id = nexuschat::chat::send_msg(&ctx, ChatId::new(chat_id), &mut msg).await?;
    ...
    // - keeping same message id as long as attachment does not change for webxnc messages
    #[expect(clippy::too_many_arguments)]
