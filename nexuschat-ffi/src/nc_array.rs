...
use crate::constants::NC_MSG_ID_DAYMARKER;
use crate::contact::ContactId;
use crate::location::Location;
use crate::message::MsgId;

/* * the structure behind nc_array_t */
#[derive(Debug, Clone)]
pub enum nc_array_t {
    MsgIds(Vec<MsgId>),
    ...
    pub(crate) fn get_id(&self, index: usize) -> u32 {
        match self {
            ...
            Self::Chat(array) => match array[index] {
                ...
                ChatItem::DayMarker { .. } => NC_MSG_ID_DAYMARKER,
            },
...
impl From<Vec<u32>> for nc_array_t {
    fn from(array: Vec<u32>) -> Self {
        nc_array_t::Uint(array)
    }
}

impl From<Vec<MsgId>> for nc_array_t {
    fn from(array: Vec<MsgId>) -> Self {
        nc_array_t::MsgIds(array)
    }
}

impl From<Vec<ContactId>> for nc_array_t {
    fn from(array: Vec<ContactId>) -> Self {
        nc_array_t::ContactIds(array)
    }
}

impl From<Vec<ChatItem>> for nc_array_t {
    fn from(array: Vec<ChatItem>) -> Self {
        nc_array_t::Chat(array)
    }
}

impl From<Vec<Location>> for nc_array_t {
    fn from(array: Vec<Location>) -> Self {
        nc_array_t::Locations(array)
    }
}
...
mod tests {
    ...
    #[test]
    fn test_nc_array() {
        let arr: nc_array_t = Vec::<u32>::new().into();
        assert!(arr.len() == 0);

        let ids: Vec<u32> = (2..1002).collect();
        let arr: nc_array_t = ids.into();
    ...
    #[test]
    #[should_panic]
    fn test_nc_array_out_of_bounds() {
        let ids: Vec<u32> = (2..1002).collect();
        let arr: nc_array_t = ids.into();
        arr.get_id(1000);
    }
