...
///
/// ```rust,norun
/// use crate::string::{nc_strdup, to_string_lossy};
/// unsafe {
///     let str_a = b"foobar\x00" as *const u8 as *const libc::c_char;
///     let str_a_copy = nc_strdup(str_a);
///     assert_eq!(to_string_lossy(str_a_copy), "foobar");
///     assert_ne!(str_a, str_a_copy);
/// }
/// ```
unsafe fn nc_strdup(s: *const libc::c_char) -> *mut libc::c_char {
...
///
/// ```
/// use nexuschat::tools::{nc_strdup, OsStrExt};
/// let path = std::path::Path::new("/some/path");
/// let path_c = path.to_c_string().unwrap();
/// unsafe {
///     let mut c_ptr: *mut libc::c_char = nc_strdup(path_c.as_ptr());
/// }
/// ```
