...
//!
//! When Nexus Chat deletes the message locally, it moves the message
...
//!
//! Vice versa, when Nexus Chat deletes the message from the server,
//! it removes IMAP folder and UID row from the `imap` table, but
//! keeps the message in the `msgs` table.
//!
//! Nexus Chat eventually removes tombstones from the `msgs` table,
...
use crate::constants::{NC_CHAT_ID_LAST_SPECIAL, NC_CHAT_ID_TRASH};
...
/// `delete_device_after` setting or `ephemeral_timestamp` column.
///
/// Emits relevant `MsgsChanged` and `WebndcInstanceDeleted` events
...
pub(crate) async fn delete_expired_messages(context: &Context, now: i64) -> Result<()> {
    ...
    if !rows.is_empty() {
        ...
        let (msgs_changed, webndc_deleted) = context
            .sql
            .transaction(|transaction| {
                let mut msgs_changed = Vec::with_capacity(rows.len());
                let mut webndc_deleted = Vec::new();
...
SELECT ?1, rfc724_mid, pre_rfc724_mid, timestamp, ? FROM msgs WHERE id=?1
                ...
                for (msg_id, chat_id, viewtype, location_id) in rows {
                    del_msg_stmt.execute((msg_id, NC_CHAT_ID_TRASH))?;
                    if location_id > 0 {
                        del_location_stmt.execute((location_id,))?;
                    }

                    msgs_changed.push((chat_id, msg_id));
                    if viewtype == Viewtype::Webndc {
                        webndc_deleted.push(msg_id)
                    }
                }
                Ok((msgs_changed, webndc_deleted))
            })
            .await?;
        ...
        for msg_id in webndc_deleted {
            context.emit_event(EventType::WebndcInstanceDeleted { msg_id });
        }
    }
    ...
    if let Some(delete_device_after) = context.get_config_delete_device_after().await? {
        ...
        let oldest_message_timestamp: Option<i64> = context
            ...
            .query_get_value(
                ...
                HAVING count(*) > 0
                "#,
                (NC_CHAT_ID_TRASH, self_chat_id, device_chat_id),
            )
            .await?;
...
async fn next_expiration_timestamp(context: &Context) -> Option<i64> {
    let ephemeral_timestamp: Option<i64> = match context
        ...
        .query_get_value(
            ...
            HAVING count(*) > 0
            "#,
            (NC_CHAT_ID_TRASH,), // Trash contains already deleted messages, skip them
        )
        .await
    {
        Err(err) => {
...
/// or previous version of Nexus Chat.
///
