...
pub trait NcKey: Serialize + Deserializable + Clone {
    ...
    fn from_slice(bytes: &[u8]) -> Result<Self> {
        ...
        // Some users that started using Nexus Chat in 2023
    ...
    fn to_base64(&self) -> String {
        base64::engine::general_purpose::STANDARD.encode(NcKey::to_bytes(self))
    }
    ...
    fn nc_fingerprint(&self) -> Fingerprint;
...
pub(crate) async fn self_fingerprint(context: &Context) -> Result<&str> {
    if let Some(fp) = context.self_fingerprint.get() {
        ...
        let fp = load_self_public_key(context).await?.nc_fingerprint().hex();
...
impl NcKey for SignedPublicKey {
    fn to_asc(&self, header: Option<(&str, &str)>) -> String {
...
impl NcKey for SignedSecretKey {
    fn to_asc(&self, header: Option<(&str, &str)>) -> String {
        // Not using .to_armored_string() to make clear *why* it is
...
pub(crate) async fn store_self_keypair(
    ...
    let new_key_id = context
        ...
        .transaction(|transaction| {
            let public_key = NcKey::to_bytes(&signed_public_key);
            let secret_key = NcKey::to_bytes(signed_secret_key);
...
Od9fIdsUaWd+Ab+svfpSoEy3wrFpP2G8340EGNBEpNcPIzqr6wQA8oRulFUMx0cS
...
-----END PGP PRIVATE KEY BLOCK-----",
        ...
        let binary = NcKey::to_bytes(&private_key);
    ...
    fn test_from_slice_roundtrip() {
        ...
        let binary = NcKey::to_bytes(&public_key);
    ...
    fn test_ignore_trailing_garbage() {
        ...
        for garbage in [
            ...
            let mut binary = NcKey::to_bytes(&private_key);
            binary.extend(garbage);

            let private_key2 =
                SignedSecretKey::from_slice(&binary).expect("Failed to ignore garbage");

            assert_eq!(private_key.nc_fingerprint(), private_key2.nc_fingerprint());
        }
    }
