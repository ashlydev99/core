...
use crate::constants::{self, Blocked, NC_VERSION_STR};
...
pub enum FolderMeaning {
    ...
    /// Nexus Chat ignores these folders because the same messages can be fetched
...
impl Imap {
    ...
    pub(crate) async fn prepare(&mut self, context: &Context) -> Result<Session> {
        ...
        let folders_configured = context
            .sql
            .get_raw_config_int(constants::NC_FOLDERS_CONFIGURED_KEY)
            .await?;
        if folders_configured.unwrap_or_default() < constants::NC_FOLDERS_CONFIGURED_VERSION {
    ...
    async fn fetch_new_msg_batch(
        ...
        for (uid, ref fetch_response) in msgs {
            ...
            // that alias `NexusChat` folder to other names.
            // For example, some Dovecot servers alias `NexusChat` folder to `INBOX.NexusChat`.
            // In this case moving from `INBOX.NexusChat` to `NexusChat`
            // results in the messages getting a new UID,
            // so the messages will be detected as new
            // in the `INBOX.NexusChat` folder again.
            ...
            for &request_uid in &request_uids {
                ...
                let received_msg = match res {
                    Err(err) => {
                        ...
                        let text = format!(
                            "❌ Failed to receive a message: {err:#}. Core version v{NC_VERSION_STR}. Please report this bug to delta@merlinux.eu or https://support.delta.chat/.",
                        );
...
impl Imap {
    pub(crate) async fn configure_folders(
        ...
        let fallback_folder = format!("INBOX{delimiter}NexusChat");
        let mvbox_folder = session
            .configure_mvbox(context, &["NexusChat", &fallback_folder])
            .await
        ...
        context
            .sql
            .set_raw_config_int(
                constants::NC_FOLDERS_CONFIGURED_KEY,
                constants::NC_FOLDERS_CONFIGURED_VERSION,
            )
            .await?;
