...
use crate::tests::pre_messages::util::{
    big_webxnc_app, send_large_file_message, send_large_image_message, send_large_webxnc_message,
};
use crate::webxnc::StatusUpdateSerial;
...
async fn test_receive_pre_message() -> Result<()> {
    ...
    // Webxnc w/o manifest.
    let (pre_message, ..) = send_large_webxnc_message(alice, alice_group_id).await?;
    let msg = bob.recv_msg(&pre_message).await;
    assert_eq!(msg.download_state, DownloadState::Available);
    assert_summary_texts(&msg, bob, "📱 test.xnc – test").await;

    let (pre_message, ..) = send_large_file_message(
        alice,
        alice_group_id,
        Viewtype::Webxnc,
        include_bytes!("../../../test-data/webxnc/timetracking-v0.10.1.xnc"),
    )
    .await?;
    let msg = bob.recv_msg(&pre_message).await;
    assert_eq!(msg.download_state, DownloadState::Available);
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_receive_webxnc() -> Result<()> {
    let mut tcm = TestContextManager::new();
    let alice = &tcm.alice().await;
    let bob = &tcm.bob().await;
    ...
    assert_summary_texts(&msg, bob, "📱 TimeTracking – test").await;
    assert_eq!(msg.get_filename().unwrap(), "test.xnc");
    Ok(())
}
...
/// Test that webxnc updates are received for pre-messages
/// and available when the Post-Message is downloaded
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_webxnc_update_for_not_downloaded_instance() -> Result<()> {
    let mut tcm = TestContextManager::new();
    let alice = &tcm.alice().await;
    let bob = &tcm.bob().await;
    let alice_group_id = alice.create_group_with_members("test group", &[bob]).await;
    ...
    let (pre_message, post_message, alice_sent_instance_msg_id) =
        send_large_webxnc_message(alice, alice_group_id).await?;
    alice
    ...
    assert_eq!(
        bob.get_webxnc_status_updates(bob_instance.id, StatusUpdateSerial::new(0))
            .await?,
        r#"[{"payload":7,"document":"doc","summary":"sum","serial":1,"max_serial":1}]"#
    );
    let info = bob_instance.get_webxnc_info(bob).await?;
...
/// Tests receiving of a large webxnc with updates attached to the the .xnc message.
///
/// Updates are sent in a post message and should be assigned to xnc instance
/// once post message is downloaded.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_webxnc_updates_in_post_message_after_pre_message() -> Result<()> {
    let mut tcm = TestContextManager::new();
    let alice = &tcm.alice().await;
    let bob = &tcm.bob().await;
    ...
    let big_webxnc_app = big_webxnc_app().await?;

    let mut alice_instance = Message::new(Viewtype::Webxnc);
    alice_instance.set_file_from_bytes(alice, "test.xnc", &big_webxnc_app, None)?;
...
/// Tests receiving of a large webxnc post-message with updates attached
/// to the the .xnc post-message when pre-message arrives later.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_webxnc_updates_in_post_message_without_pre_message() -> Result<()> {
    let mut tcm = TestContextManager::new();
    let alice = &tcm.alice().await;
    let bob = &tcm.bob().await;
