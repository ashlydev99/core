...
use crate::key::{NcKey, load_self_public_key};
...
async fn check_verified_oneonone_chat_protection_not_broken(by_classical_email: bool) {
    ...
    if by_classical_email {
        ...
        tcm.section("Bob sets up another Nexus Chat device");
    ...
    tcm.section("Bob sends another message from NC");
    SystemTime::shift(std::time::Duration::from_secs(3600));
    tcm.send_recv(&bob, &alice, "Using NC again").await;
...
async fn test_create_verified_oneonone_chat() -> Result<()> {
    ...
    tcm.section("Fiona reinstalls NC");
    drop(fiona);
...
async fn test_outgoing_mua_msg() -> Result<()> {
    ...
    tcm.send_recv_accept(&bob, &alice, "Heyho from NC").await;
    assert_verified(&alice, &bob).await;
    ...
    .await?
    .unwrap();
    tcm.send_recv(&alice, &bob, "Sending with NC again").await;
...
/// Tests that message from old DN setup does not break
/// new verified chat.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_message_from_old_nc_setup() -> Result<()> {
    let mut tcm = TestContextManager::new();
    let alice = &tcm.alice().await;
...
/// Tests handling of resent .xnc arriving before "Member added"
/// in a verified group
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_verified_chat_editor_reordering() -> Result<()> {
    ...
    tcm.section("Bob sends an .xmc to the chat");

    let mut webxnc_instance = Message::new(Viewtype::File);
    webxnc_instance.set_file_from_bytes(
        bob,
        "editor.xnc",
        include_bytes!("../../test-data/webxnc/minimal.xnc"),
        None,
    )?;
    let bob_instance_msg_id = send_msg(bob, bob_chat_id, &mut webxnc_instance).await?;
    let bob_sent_instance_msg = bob.pop_sent_msg().await;

    tcm.section("Alice receives .xnc");
    ...
    tcm.section("Bob resends webxnc");
    resend_msgs(bob, &[bob_instance_msg_id]).await?;

    tcm.section("Charlie receives resent webxnc before member added");
    let charlie_received_xnc = charlie.recv_msg(&bob.pop_sent_msg().await).await;
