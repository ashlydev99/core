...
use crate::constants::NC_CHAT_ID_TRASH;
...
async fn test_setup_contact_bob_knows_alice() -> Result<()> {
    ...
    assert!(msg.get_header(HeaderDef::SecureJoinAuth).is_some());
    let bob_fp = load_self_public_key(bob).await?.nc_fingerprint();
    assert_eq!(
        *msg.get_header(HeaderDef::SecureJoinFingerprint).unwrap(),
        bob_fp.hex()
    );
...
async fn test_secure_join_group_ex(v3: bool, remove_invite: bool) -> Result<()> {
    ...
    // Old Nexus Chat core sent `Secure-Join-Group` header in `vg-request`,
    // but it was only used by Alice in `vg-request-with-auth`.
    // New Nexus Chat versions do not use `Secure-Join-Group` header at all
