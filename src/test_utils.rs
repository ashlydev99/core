...
use chat::ChatItem;
use nexuschat_contact_tools::{ContactAddress, EmailAddress};
...
impl TestContext {
    ...
    pub async fn recv_msg_opt(
        ...
        receive_imf(self, msg.payload().as_bytes(), false)
            .await
            .unwrap()
            .filter(|msg| msg.chat_id != NC_CHAT_ID_TRASH)
    }
    ...
    pub async fn recv_msg_trash(&self, msg: &SentMessage<'_>) {
        let received = receive_imf(self, msg.payload().as_bytes(), false)
            ...
            .unwrap()
            .unwrap();
        assert_eq!(received.chat_id, NC_CHAT_ID_TRASH);
    }
    ...
    pub async fn get_last_msg(&self) -> Message {
        let chats = Chatlist::try_load(&self.ctx, NC_GCL_NO_SPECIALS, None, None)
            .await
            .expect("failed to load chatlist");
        // 0 is correct in the next line (as opposed to `chats.len() - 1`, which would be the last element):
        // The chatlist describes what you see when you open NC, a list of chats and in each of them
...
///
/// This is a raw message, probably in the shape NC was planning to send it but not having
