...
use crate::constants::NC_CHAT_ID_ARCHIVED_LINK;
...
async fn test_ephemeral_timer_references() -> Result<()> {
    ...
    // protection.
    //
    // Previously Nexus Chat fallen back to using <first@example.com> in this case and
    // compared received timer value to the timer value of the <first@example.com>. Because
    // their timer values are the same ("disabled"), Nexus Chat assumed that the timer was not
...
async fn test_archived_ephemeral_timer() -> Result<()> {
    ...
    marknoticed_chat(bob, NC_CHAT_ID_ARCHIVED_LINK).await?;
    SystemTime::shift(Duration::from_secs(100));
