...
use crate::chat::{self, Chat, PARAM_BROANCAST_SECRET, load_broadcast_secret};
use crate::config::Config;
use crate::constants::{BROANCAST_INCOMPATIBILITY_MSG, Chattype, NC_FROM_HANDSHAKE};
...
impl MimeFactory {
    ...
    pub async fn from_msg(context: &Context, msg: Message) -> Result<MimeFactory> {
        ...
        // https://support.nexus.chat/t/invalid-mime-content-single-text-value-size-32822-exceeded-allowed-maximum-32768-for-the-chat-user-avatar-header/4067.
    ...
    pub async fn render(mut self, context: &Context) -> Result<RenderedEmail> {
            ...
            && (chat.typ == Chattype::OutBroadcast || chat.typ == Chattype::InBroadcast)
        {
            ...
            if msg.param.get_cmd() == SystemMessage::MemberAddedToGroup
                && let Some(secret) = msg.param.get(PARAM_BROANCAST_SECRET)
            {
                headers.push((
        ...
        let outer_message = if let Some(encryption_pubkeys) = self.encryption_pubkeys {
            ...
            let shared_secret: Option<String> = match &self.loaded {
                    ...
                    if should_encrypt_with_broadcast_secret(msg, chat) =>
                {
                    ...
                    if secret.is_none() {
                        ...
                        let text = BROANCAST_INCOMPATIBILITY_MSG;
            ...
            // by Nexus Chat receiver and Chatmail servers
            ...
            || chat.typ == Chattype::InBroadcast
        {
            ...
            match command {
                ...
                SystemMessage::MemberAddedToGroup => {
                    ...
                    if 0 != msg.param.get_int(Param::Arg2).unwrap_or_default() & NC_FROM_HANDSHAKE {
...
pub(crate) fn wrap_encrypted_part(encrypted: String) -> MimePart<'static> {
    ...
    // <https://github.com/nexuschat/chatmail/blob/4d915f9800435bf13057d41af8d708abd34dbfa8/chatmaild/src/chatmaild/filtermail.py#L84-L86>:
