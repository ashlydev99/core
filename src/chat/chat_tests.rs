...
use crate::constants::{NC_GCL_ARCHIVED_ONLY, NC_GCL_NO_SPECIALS};
...
async fn test_get_draft_special_chat_id() {
    let t = TestContext::new().await;
    let draft = NC_CHAT_ID_LAST_SPECIAL.get_draft(&t).await.unwrap();
    assert!(draft.is_none());
}
...
async fn test_archive() {
    ...
    assert_eq!(chatlist_len(&t, NC_GCL_NO_SPECIALS).await, 2);
    assert_eq!(chatlist_len(&t, NC_GCL_ARCHIVED_ONLY).await, 0);
    assert_eq!(NC_GCL_ARCHIVED_ONLY, 0x01);
    assert_eq!(NC_GCL_NO_SPECIALS, 0x02);
    ...
    assert_eq!(get_chat_cnt(&t).await.unwrap(), 2);
    assert_eq!(chatlist_len(&t, 0).await, 2); // including NC_CHAT_ID_ARCHIVED_LINK now
    assert_eq!(chatlist_len(&t, NC_GCL_NO_SPECIALS).await, 1);
    assert_eq!(chatlist_len(&t, NC_GCL_ARCHIVED_ONLY).await, 1);
    ...
    assert_eq!(get_chat_cnt(&t).await.unwrap(), 2);
    assert_eq!(chatlist_len(&t, 0).await, 1); // only NC_CHAT_ID_ARCHIVED_LINK now
    assert_eq!(chatlist_len(&t, NC_GCL_NO_SPECIALS).await, 0);
    assert_eq!(chatlist_len(&t, NC_GCL_ARCHIVED_ONLY).await, 2);
...
async fn test_marknoticed_all_chats() -> Result<()> {
    ...
    for event in &[
        ...
        EventType::ChatlistItemChanged {
            chat_id: Some(NC_CHAT_ID_ARCHIVED_LINK),
        },
    ] {
...
async fn test_archive_fresh_msgs() -> Result<()> {
    ...
    bob_chat_id
        .set_visibility(&t, ChatVisibility::Archived)
        .await?;
    assert_eq!(NC_CHAT_ID_ARCHIVED_LINK.get_fresh_msg_cnt(&t).await?, 0);

    msg_from(&t, "bob", 2).await?;
    assert_eq!(NC_CHAT_ID_ARCHIVED_LINK.get_fresh_msg_cnt(&t).await?, 1);
    ...
    assert_eq!(NC_CHAT_ID_ARCHIVED_LINK.get_fresh_msg_cnt(&t).await?, 2);
        ...
        .get_matching(|ev| {
            matches!(
                ...
                EventType::MsgsChanged {
                    chat_id: NC_CHAT_ID_ARCHIVED_LINK,
                    ..
                }
            )
        })
    ...
    marknoticed_chat(&t, NC_CHAT_ID_ARCHIVED_LINK).await?;
...
async fn test_pinned() {
    ...
    let chatlist = get_chats_from_chat_list(&t, NC_GCL_NO_SPECIALS).await;
...
async fn test_contact_request_archive() -> Result<()> {
    ...
    let chats = Chatlist::try_load(&t, NC_GCL_ARCHIVED_ONLY, None, None).await?;
    assert_eq!(chats.len(), 1);
    let chat_id = chats.get_chat_id(0)?;
    assert!(Chat::load_from_db(&t, chat_id).await?.is_contact_request());
...
async fn test_broadcast_change_name() -> Result<()> {
...

    {
        ...
        // older versions of NC would ignore the message
...
async fn test_broadcast_recipients_sync1() -> Result<()> {
    ...
    // (e.g. because alice2 runs an older version of NC),
...
async fn test_block_broadcast() -> Result<()> {
    ...
    let chats = Chatlist::try_load(bob, NC_GCL_NO_SPECIALS, None, None).await?;
    assert_eq!(chats.len(), 1);
...
async fn test_broadcast_channel_protected_listid() -> Result<()> {
    ...
    // Check that Nexus Chat ignores an injected List-ID header:
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_get_chat_media_webxnc_order() -> Result<()> {
    let mut tcm = TestContextManager::new();
    let alice = tcm.alice().await;
    let bob = tcm.bob().await;
    let chat = alice.create_chat(&bob).await;

    let mut instance1 = Message::new(Viewtype::Webxnc);
    instance1.set_file_from_bytes(
        &alice,
        "test1.xnc",
        include_bytes!("../../test-data/webxnc/minimal.xdc"),
        None,
    )?;
...
async fn test_send_edit_request() -> Result<()> {
    ...
    send_edit_request(alice, alice_msg.id, "Text me on Nexus.Chat".to_string()).await?;
    let sent2 = alice.pop_sent_msg().await;
    let test = Message::load_from_db(alice, alice_msg.id).await?;
    assert_eq!(test.text, "Text me on Nexus.Chat");
