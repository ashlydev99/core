...
use chrono::TimeZone;
use nexuschat_contact_tools::{ContactAddress, sanitize_bidi_characters, sanitize_single_line};
...
use crate::constants::{
    Blocked, Chattype, NC_CHAT_ID_ALLDONE_HINT, NC_CHAT_ID_ARCHIVED_LINK, NC_CHAT_ID_LAST_SPECIAL,
    NC_CHAT_ID_TRASH, NC_RESEND_USER_AVATAR_DAYS, EDITED_PREFIX, TIMESTAMP_SENT_TOLERANCE,
};
use crate::contact::{self, Contact, ContactId, Origin};
use crate::context::Context;
use crate::debug_logging::maybe_set_logging_ndc;
use crate::download::{
...
use crate::webndc::StatusUpdateSerial;

pub(crate) const PARAM_BROANCAST_SECRET: Param = Param::Arg3;
...
impl ChatId {
    ...
    pub fn is_special(self) -> bool {
        (0..=NC_CHAT_ID_LAST_SPECIAL.0).contains(&self.0)
    }
    ...
    /// Also emits NC_EVENT_MSGS_CHANGED for NC_CHAT_ID_ARCHIVED_LINK when the number of archived
    ...
    pub async fn unarchive_if_not_muted(
        ...
        if chat.is_muted() {
            ...
            if unread_cnt == 1 {
                ...
                context.emit_msgs_changed_without_msg_id(NC_CHAT_ID_ARCHIVED_LINK);
            }
            return Ok(());
        }
        context
            ...
            _ => {
                if msg.viewtype == Viewtype::File
                    && let Some((better_type, _)) = message::guess_msgtype_from_suffix(msg)
                        ...
                        .filter(|&(vt, _)| vt == Viewtype::Webndc || vt == Viewtype::Vcard)
                {
        ...
        let chat_size: HashMap<ChatId, f64> = context
            ...
            .query_map_collect(
                "SELECT chat_id, count(*) AS n
                 ...
                 GROUP BY chat_id",
                (ContactId::LAST_SPECIAL, NC_CHAT_ID_LAST_SPECIAL),
                |row| {
                    let chat_id: ChatId = row.get(0)?;
...
impl Chat {
    ...
    /// nexuschat, and the data returned is still subject to change.
    ...
    async fn prepare_msg_raw(
        ...
        if let Some(update_msg_id) = update_msg_id {
            ...
            maybe_set_logging_ndc(context, msg, self.id).await?;
            context
                .update_webndc_integration_database(msg, context)
                .await?;
        }
...
pub struct ChatInfo {
    ...
    /// `NC_CHAT_TYPE_UNDEFINED`, `NC_CHAT_TYPE_SINGLE`,
    /// or `NC_CHAT_TYPE_GROUP`
    /// constants.
    #[serde(rename = "type")]
    pub type_: u32,
            ...
            .query_row_optional(
                "SELECT c.id, c.blocked
                  ...
                  WHERE c.type=100  -- 100 = Chattype::Single
                    AND c.id>9      -- 9 = NC_CHAT_ID_LAST_SPECIAL
                    AND j.contact_id=?;",
                (contact_id,),
                |row| {
...
async fn prepare_msg_blob(context: &Context, msg: &mut Message) -> Result<()> {
    if msg.viewtype == Viewtype::Text || msg.viewtype == Viewtype::Call {
            ...
            || msg.viewtype == Viewtype::Sticker && !msg.param.exists(Param::ForceSticker)
        {
            ...
            if let Some((better_type, _)) = message::guess_msgtype_from_suffix(msg) {
                if msg.viewtype == Viewtype::Sticker {
                    ...
                    maybe_image = true;
                } else if better_type != Viewtype::Webndc
                    || context
                        .ensure_sendable_webndc_file(&blob.to_abs_path())
                        .await
                        .is_ok()
                {
                    msg.viewtype = better_type;
        ...
        } else if msg.viewtype == Viewtype::Webndc {
            context
                .ensure_sendable_webndc_file(&blob.to_abs_path())
                .await?;
        }
...
pub(crate) async fn create_send_msg_jobs(context: &Context, msg: &mut Message) -> Result<Vec<i64>> {
    ...
    // Default Webndc integrations are hidden messages and must not be sent out:
    if (msg.param.get_int(Param::WebndcIntegration).is_some() && msg.hidden)
...
pub async fn send_edit_request(context: &Context, msg_id: MsgId, new_text: String) -> Result<()> {
    ...
    let mut edit_msg = Message::new_text(EDITED_PREFIX.to_owned() + &new_text); // prefix only set for nicer display in Non-nexus-MUAs
    edit_msg.set_quote(context, Some(&original_msg)).await?; // quote only set for nicer display in Non-nexus-MUAs
...
pub async fn get_chat_media(
    ...
    let list = if msg_type == Viewtype::Webndc
...
const SQL_INSERT_BROANCAST_SECRET: &str =
...
pub(crate) async fn create_out_broadcast_ex(
    ...
    let trans_fn = |t: &mut rusqlite::Transaction| -> Result<ChatId> {
        ...
        t.execute(SQL_INSERT_BROANCAST_SECRET, (chat_id, &secret))?;
        Ok(chat_id)
    };
...
pub(crate) async fn save_broadcast_secret(
    ...
    context
        .sql
        .execute(SQL_INSERT_BROANCAST_SECRET, (chat_id, secret))
        .await?;
...
pub(crate) async fn add_contact_to_chat_ex(
    ...
    if chat.is_promoted() {
        ...
        if chat.typ == Chattype::OutBroadcast {
            ...
            msg.param.set(PARAM_BROANCAST_SECRET, secret);
        }
        send_msg(context, chat_id, &mut msg).await?;
...
pub(crate) async fn shall_attach_selfavatar(context: &Context, chat_id: ChatId) -> Result<bool> {
    let timestamp_some_days_ago = time() - NC_RESEND_USER_AVATAR_DAYS * 24 * 60 * 60;
...
pub(crate) async fn save_copy_in_self_talk(
    ...
    msg.param.remove(Param::WebndcDocument);
    msg.param.remove(Param::WebndcDocumentTimestamp);
    msg.param.remove(Param::WebndcSummary);
    msg.param.remove(Param::WebndcSummaryTimestamp);
...
///
/// This is primarily intended to make existing webndcs available to new chat members.
pub async fn resend_msgs(context: &Context, msg_ids: &[MsgId]) -> Result<()> {
    ...
    for mut msg in msgs {
        ...
        if msg.viewtype == Viewtype::Webndc {
...
impl Context {
    ...
    pub(crate) fn on_archived_chats_maybe_noticed(&self) {
        self.emit_msgs_changed_without_msg_id(NC_CHAT_ID_ARCHIVED_LINK);
    }
}
