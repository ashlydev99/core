...
use nexuschat_contact_tools::addr_cmp;
    ...
    let mut verified_key_contacts: BTreeMap<u32, u32> = BTreeMap::new();
    {
        ...
        for row in all_address_contacts? {
            ...
            let mut insert_contact = |key: SignedPublicKey| -> Result<u32> {
                let fingerprint = key.nc_fingerprint().hex();
            ...
            let verifier_id = if addr_cmp(&verifier, &addr) {
                // Earlier versions of Nexus Chat signalled a direct verification
...
ALTER TABLE msgs ADD COLUMN mime_references TEXT;"#,
    ...
    if dbversion < 50 {
        // installations <= 0.100.1 used NC_SHOW_EMAILS_ALL implicitly;
        // keep this default and use NC_SHOW_EMAILS_NO
    ...
    if dbversion < 91 {
        sql.execute_migration(
            r#"CREATE TABLE smtp_status_updates (
              msg_id INTEGER NOT NULL UNIQUE, -- msg_id of the webxnc instance with pending updates
...
ALTER TABLE contacts ADD COLUMN name_normalized TEXT;
    ...
    if dbversion < migration_version {
        ...
        // but we still keep it so that it's possible to import a backup into an older NC version,
