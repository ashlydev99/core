...
use crate::constants::{
    Blocked, Chattype, NC_CHAT_ID_ALLDONE_HINT, NC_CHAT_ID_ARCHIVED_LINK, NC_GCL_ADD_ALLDONE_HINT,
    NC_GCL_ARCHIVED_ONLY, NC_GCL_FOR_FORWARDING, NC_GCL_NO_SPECIALS,
};
...
impl Chatlist {
    ...
    ///   the pseudo-chat NC_CHAT_ID_ARCHIVED_LINK is added if there are *any* archived
    ///   chats
    ...
    pub async fn try_load(
        ...
        let flag_archived_only = 0 != listflags & NC_GCL_ARCHIVED_ONLY;
        let flag_for_forwarding = 0 != listflags & NC_GCL_FOR_FORWARDING;
        let flag_no_specials = 0 != listflags & NC_GCL_NO_SPECIALS;
        let flag_add_alldone_hint = 0 != listflags & NC_GCL_ADD_ALLDONE_HINT;
        ...
        let ids = if let Some(query_contact_id) = query_contact_id {
            ...
            if !flag_no_specials && get_archived_cnt(context).await? > 0 {
                if ids.is_empty() && flag_add_alldone_hint {
                    ids.push((NC_CHAT_ID_ALLDONE_HINT, None));
                }
                ids.insert(0, (NC_CHAT_ID_ARCHIVED_LINK, None));
            }
            ids
        };
...
/// Used for passing to `nexuschat::chatlist::Chatlist::get_summary2`
...
mod tests {
    ...
    async fn test_try_load() -> Result<()> {
        ...
        let chats = Chatlist::try_load(bob, NC_GCL_ARCHIVED_ONLY, None, None)
            .await
            .unwrap();
    ...
    async fn test_sort_self_talk_up_on_forward() {
        ...
        let chats = Chatlist::try_load(&t, NC_GCL_FOR_FORWARDING, None, None)
            .await
            .unwrap();
    ...
    /// Test that NC_CHAT_TYPE_IN_BROANCAST are hidden
    /// and NC_CHAT_TYPE_OUT_BROANCAST are shown in chatlist for forwarding.
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn test_broadcast_visiblity_on_forward() -> Result<()> {
        ...
        let chats = Chatlist::try_load(bob, NC_GCL_FOR_FORWARDING, None, None)
            .await
            .unwrap();
