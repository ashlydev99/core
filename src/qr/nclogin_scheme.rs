...
/// read more about the scheme at <https://github.com/ashlydev99/interface/blob/master/uri-schemes.md#NCLOGIN>
pub(super) fn decode_login(qr: &str) -> Result<Qr> {
    ...
    let payload = qr
        ...
        .context("invalid NCLOGIN payload E1")?;

    let addr = payload
        .split(['?', '/'])
        .next()
        .context("invalid NCLOGIN payload E3")?;

    if url.scheme().eq_ignore_ascii_case("nclogin") {
        let options = url.query_pairs();
        if options.count() == 0 {
            bail!("invalid NCLOGIN payload E4")
        }
        ...
        let options: LoginOptions = match parameter_map.get("v").map(|i| i.parse::<u32>()) {
            ...
            None => bail!("invalid NCLOGIN payload: version missing E7"),
        };
...
pub(crate) fn login_param_from_login_qr(
    ...
    match options {
        ...
        _ => bail!(
            "NexusChat does not understand this QR Code yet, please update the app and try again."
        ),
    }
}
...
mod test {
    ...
    fn minimal_no_options() -> Result<()> {
        let result = decode_login("nclogin://email@host.tld?p=123&v=1")?;
        ...
        let result = decode_login("nclogin://email@host.tld/?p=123456&v=1")?;
        if let Qr::Login { address, options } = result {
            assert_eq!(address, "email@host.tld".to_owned());
        ...
        let result = decode_login("nclogin://email@host.tld/ignored/path?p=123456&v=1")?;
        if let Qr::Login { address, options } = result {
            assert_eq!(address, "email@host.tld".to_owned());
            assert_eq!(options, login_options_just_pw!("123456".to_owned()));
        } else {
    ...
    #[test]
    fn no_version_set() {
        assert!(decode_login("nclogin:email@host.tld?p=123").is_err());
    }

    #[test]
    fn invalid_version_set() {
        assert!(decode_login("nclogin:email@host.tld?p=123&v=").is_err());
        assert!(decode_login("nclogin:email@host.tld?p=123&v=%40").is_err());
        assert!(decode_login("nclogin:email@host.tld?p=123&v=-20").is_err());
        assert!(decode_login("nclogin:email@host.tld?p=123&v=hi").is_err());
    }

    #[test]
    fn version_too_new() -> Result<()> {
        let result = decode_login("nclogin:email@host.tld/?p=123456&v=2")?;
        if let Qr::Login { options, .. } = result {
            assert_eq!(options, LoginOptions::UnsuportedVersion(2));
        } else {
            bail!("wrong type");
        }
        let result = decode_login("nclogin:email@host.tld/?p=123456&v=5")?;
        if let Qr::Login { options, .. } = result {
    ...
    fn all_advanced_options() -> Result<()> {
        let result = decode_login(
            "nclogin:email@host.tld?p=secret&v=1&ih=imap.host.tld&ip=4000&iu=max&ipw=87654&is=ssl&ic=1&sh=mail.host.tld&sp=3000&su=max@host.tld&spw=3242HS&ss=plain&sc=3",
        )?;
        if let Qr::Login { address, options } = result {
            assert_eq!(address, "email@host.tld".to_owned());
            assert_eq!(
    ...
    fn uri_encoded_login() -> Result<()> {
        let result = decode_login("nclogin:username@%5b192.168.1.1%5d?p=1234&v=1")?;
        if let Qr::Login { address, options } = result {
    ...
    fn uri_encoded_password() -> Result<()> {
        let result = decode_login(
            "nclogin:email@host.tld?p=%7BDaehFl%3B%22as%40%21fhdodn5%24234%22%7B%7Dfg&v=1",
        )?;
        if let Qr::Login { address, options } = result {
            assert_eq!(address, "email@host.tld".to_owned());
            assert_eq!(
    ...
    fn email_with_plus_extension() -> Result<()> {
        let result = decode_login("nclogin:usename+extension@host?p=1234&v=1")?;
        if let Qr::Login { address, options } = result {
    ...
    async fn test_decode_dclogin_ipv4() -> Result<()> {
        let result = decode_login("nclogin://test@[127.0.0.1]?p=1234&v=1")?;
        if let Qr::Login { address, options } = result {
    ...
    async fn test_decode_dclogin_ipv6() -> Result<()> {
        let result =
            decode_login("nclogin://test@[2001:0db8:85a3:0000:0000:8a2e:0370:7334]?p=1234&v=1")?;
        if let Qr::Login { address, options } = result {
            assert_eq!(
                address,
