...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_decode_inexuschat_link() -> Result<()> {
    ...
    for ctx in &[ctx_configured, ctx_unconfigured] {
        let qr = check_qr(
            ctx,
            "https://i.nexus.chat/#79252762C34C5096AF57958F4FC3D21A81B0F0A7&a=cli%40nexuschat.de&g=test%20%3F+test%20%21&x=h-0oKQf2CDK&i=9JEXlxAqGM0&s=0V7LzL9cxRL"
        ).await?;
...
// see issue https://github.com/ashlydev99/nexuschat-core-rust/issues/1969 for more info
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_decode_openpgp_tolerance_for_issue_1969() -> Result<()> {
    ...
    let qr = check_qr(
        &ctx.ctx,
        "OPENPGP4FPR:79252762C34C5096AF57958F4FC3D21A81B0F0A7%23a=cli%40nexuschat.de&g=test%20%3F+test%20%21&x=h-0oKQf2CDK&i=9JEXlxAqGM0&s=0V7LzL9cxRL"
    ).await?;
...
async fn test_decode_openpgp_group() -> Result<()> {
    ...
    let qr = check_qr(
        &ctx.ctx,
        "OPENPGP4FPR:79252762C34C5096AF57958F4FC3D21A81B0F0A7#a=cli%40nexuschat.de&g=test%20%3F+test%20%21&x=h-0oKQf2CDK&i=9JEXlxAqGM0&s=0V7LzL9cxRL"
    ).await?;
    ...
    } = qr
    {
        ...
        let contact = Contact::get_by_id(&ctx.ctx, contact_id).await?;
        assert_eq!(contact.get_addr(), "cli@nexuschat.de");
    } else {
        bail!("Wrong QR code type");
    }
...
async fn test_decode_openpgp_invalid_token() -> Result<()> {
    ...
    let qr = check_qr(
        &ctx.ctx,
        "OPENPGP4FPR:79252762C34C5096AF57958F4FC3D21A81B0F0A7#a=cli%40nexuschat.de&g=test%20%3F+test%20%21&x=h-0oKQf2CDK&i=9JEXlxAqGM0&s=0V7LzL/cxRL"
    ).await?;
...
async fn test_decode_openpgp_secure_join() -> Result<()> {
    ...
    let qr = check_qr(
        &ctx.ctx,
        "OPENPGP4FPR:79252762C34C5096AF57958F4FC3D21A81B0F0A7#a=cli%40nexuschat.de&n=J%C3%B6rn%20P.+P.&i=TbnwJ6lSvD5&s=0ejvbdFSQxB"
    ).await?;
    ...
    let qr = check_qr(
        &ctx.ctx,
        "openpgp4fpr:79252762C34C5096AF57958F4FC3D21A81B0F0A7#a=cli%40nexuschat.de&n=&i=TbnwJ6lSvD5&s=0ejvbdFSQxB"
    ).await?;
