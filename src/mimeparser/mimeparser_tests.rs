...
use crate::{
    chat,
    chatlist::Chatlist,
    constants::{self, Blocked, NC_DESIRED_TEXT_LEN, NC_ELLIPSIS},
    key,
...
\n\
Reporting-UA: Nexus Chat 1.0.0-beta.22\n\
...
Subject: Nexus Chat is great stuff!
...
"#;
    ...
    assert_eq!(
        message.get_subject(),
        Some("Nexus Chat is great stuff!".to_string())
    );

    assert_eq!(message.parts.len(), 1);
    assert_eq!(message.parts[0].typ, Viewtype::Image);
    assert_eq!(message.parts[0].msg, "Nexus Chat is great stuff! – Test");
}
...
async fn test_mime_modified_alt_plain() {
    ...
    assert_eq!(
        ...
        although not being a nexus-message.\n\
...
async fn test_mime_modified_large_plain() -> Result<()> {
...

    {
        ...
        assert!(
            mimemsg.parts[0].msg.matches("just repeated").count()
                <= NC_DESIRED_TEXT_LEN / REPEAT_TXT.len()
        );
        assert!(mimemsg.parts[0].msg.len() <= NC_DESIRED_TEXT_LEN + NC_ELLIPSIS.len());
    }
    ...
    for draft in [false, true] {
        ...
        assert!(
            msg.text.matches("just repeated.").count() <= NC_DESIRED_TEXT_LEN / REPEAT_TXT.len()
        );
        assert!(msg.text.len() <= NC_DESIRED_TEXT_LEN + NC_ELLIPSIS.len());
...
async fn test_intended_recipient_fingerprint() -> Result<()> {
    let mut tcm = TestContextManager::new();
    let t = &tcm.alice().await;
    let t_fp = key::load_self_public_key(t).await?.nc_fingerprint();
...
";
    ...
    // Nexus Chat generates In-Reply-To with a starting tab when Message-ID is too long.
...
/// Tests that Nexus Chat takes the last header value
...
/// Tests that loading a bobstate from an old version of Nexus Chat
