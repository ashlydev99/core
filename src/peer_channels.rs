//! Peer channels for realtime communication in webndcs.
//!
//! We use Iroh as an ephemeral peer channels provider to create direct communication
//! channels between webndcs. See [here](https://webndc.org/docs/spec/joinRealtimeChannel.html) for the webndc specs.
//!
//! Ephemeral channels should be established lazily, to avoid bootstrapping p2p connectivity
//! when it's not required. Only when a webndc subscribes to realtime data or when a reatlime message is sent,
//! the p2p machinery should be started.
//!
//! Adding peer channels to webndc needs upfront negotiation of a topic and sharing of public keys so that
...
//!    securely generated on the initial webndc sender's device. This message header is encrypted
//!    and sent in the same message as the webndc application.
//! 2. Whenever `joinRealtimeChannel().setListener()` or `joinRealtimeChannel().send()` is called by the webndc application,
...
impl Iroh {
    ...
    pub async fn send_webndc_realtime_data(
        &self,
        ctx: &Context,
        msg_id: MsgId,
        mut data: Vec<u8>,
...
impl Context {
    ...
    pub async fn get_or_try_init_peer_channel(
        ...
        if !self.get_config_bool(Config::WebndcRealtimeEnabled).await? {
...
pub async fn add_gossip_peer_from_header(
    ...
    if !context
        .get_config_bool(Config::WebndcRealtimeEnabled)
        .await?
    {
        return Ok(());
    }
    ...
    context.emit_event(EventType::WebndcRealtimeAdvertisementReceived {
        msg_id: instance_id,
    });
...
/// Get a list of [NodeAddr]s for one webndc.
...
pub async fn send_webndc_realtime_advertisement(
    ctx: &Context,
    msg_id: MsgId,
) -> Result<Option<oneshot::Receiver<()>>> {
    if !ctx.get_config_bool(Config::WebndcRealtimeEnabled).await? {
        return Ok(None);
    }
    ...
    let webndc = Message::load_from_db(ctx, msg_id).await?;
    let mut msg = Message::new(Viewtype::Text);
    msg.hidden = true;
    msg.param.set_cmd(SystemMessage::IrohNodeAddr);
    msg.in_reply_to = Some(webndc.rfc724_mid.clone());
    send_msg(ctx, webndc.chat_id, &mut msg).await?;
    info!(ctx, "IROH_REALTIME: Sent realtime advertisement");
    Ok(conn)
}

/// Send realtime data to other peers using iroh.
pub async fn send_webndc_realtime_data(ctx: &Context, msg_id: MsgId, data: Vec<u8>) -> Result<()> {
    if !ctx.get_config_bool(Config::WebndcRealtimeEnabled).await? {
        return Ok(());
    }

    let iroh = ctx.get_or_try_init_peer_channel().await?;
    iroh.send_webndc_realtime_data(ctx, msg_id, data).await?;
    Ok(())
}

/// Leave the gossip of the webndc with given [MsgId].
///
/// NB: When this is called before closing a webndc app in UIs, it must be guaranteed that
/// `send_webndc_realtime_*()` functions aren't called for the given `msg_id` anymore until the app
/// is open again.
pub async fn leave_webndc_realtime(ctx: &Context, msg_id: MsgId) -> Result<()> {
        ...
        match event {
            Event::Gossip(event) => match event {
                ...
                GossipEvent::Received(message) => {
                    ...
                    context.emit_event(EventType::WebndcRealtimeData {
                        msg_id,
                        data: message
                            .content
...
mod tests {
    ...
    async fn test_can_communicate() {
        ...
        instance
            .set_file_from_bytes(
                alice,
                "minimal.ndc",
                include_bytes!("../test-data/webndc/minimal.ndc"),
                None,
            )
            .unwrap();

        send_msg(alice, alice_chat.id, &mut instance).await.unwrap();
        let alice_webndc = alice.get_last_msg().await;
        assert_eq!(alice_webndc.get_viewtype(), Viewtype::Webndc);

        let webndc = alice.pop_sent_msg().await;
        let bob_webndc = bob.recv_msg(&webndc).await;
        assert_eq!(bob_webndc.get_viewtype(), Viewtype::Webndc);

        bob_webndc.chat_id.accept(bob).await.unwrap();

        // Alice advertises herself.
        send_webndc_realtime_advertisement(alice, alice_webndc.id)
            .await
            .unwrap();
        ...
        loop {
            ...
            if let EventType::WebndcRealtimeAdvertisementReceived { msg_id } = event.typ {
                assert!(msg_id == bob_webndc.id);
                break;
            }
        }

        // Bob adds alice to gossip peers.
        let members = get_iroh_gossip_peers(bob, bob_webndc.id)
            .await
            .unwrap()
            .into_iter()
        ...
        bob.get_or_try_init_peer_channel()
            .await
            .unwrap()
            .join_and_subscribe_gossip(bob, bob_webndc.id)
            .await
            .unwrap()
            .unwrap()
            .await
        ...
        alice
            ...
            .await
            .unwrap()
            .send_webndc_realtime_data(alice, alice_webndc.id, "alice -> bob".as_bytes().to_vec())
            .await
            .unwrap();

        loop {
            let event = bob.evtracker.recv().await.unwrap();
            if let EventType::WebndcRealtimeData { data, .. } = event.typ {
        ...
        bob.get_or_try_init_peer_channel()
            .await
            .unwrap()
            .send_webndc_realtime_data(bob, bob_webndc.id, "bob -> alice".as_bytes().to_vec())
            .await
            .unwrap();
        ...
        let members = get_iroh_gossip_peers(alice, alice_webndc.id)
            .await
            .unwrap()
            .into_iter()
            .map(|addr| addr.node_id)
        ...
        bob.get_or_try_init_peer_channel()
            .await
            .unwrap()
            .send_webndc_realtime_data(bob, bob_webndc.id, "bob -> alice 2".as_bytes().to_vec())
            .await
            .unwrap();
    ...
    async fn test_can_reconnect() {
        ...
        let bob_topic = get_iroh_topic_for_msg(bob, bob_webndc.id)
            .await
            .unwrap()
            .unwrap();
        let bob_sequence_number = bob
            ...
            .get(&bob_topic)
            .copied();
        leave_webndc_realtime(bob, bob_webndc.id).await.unwrap();
        ...
        leave_webndc_realtime(alice, alice_webndc.id).await.unwrap();
        let topic = get_iroh_topic_for_msg(alice, alice_webndc.id)
            .await
            .unwrap()
            .unwrap();
        assert!(
    ...
    async fn test_webndc_resend() {
        ...
        let fiona_connect_future = send_webndc_realtime_advertisement(fiona, fiona_instance.id)
            .await
            .unwrap()
            .unwrap();
        ...
        let realtime_send_loop = async {
            ...
            loop {
                send_webndc_realtime_data(alice, instance.id, b"alice -> bob & fiona".into())
                    .await
                    .unwrap();
    ...
    async fn connect_alice_bob(
        ...
        // Alice advertises herself.
        let alice_advertisement_future = send_webndc_realtime_advertisement(alice, alice_webndc.id)
            .await
            .unwrap()
            .unwrap();
        let alice_advertisement = alice.pop_sent_msg().await;

        let bob_advertisement_future = send_webndc_realtime_advertisement(bob, bob_webndc.id)
            .await
            .unwrap()
            .unwrap();
        ...
        send_webndc_realtime_data(alice, alice_webndc.id, b"alice -> bob".into())
            .await
            .unwrap();
    ...
    async fn test_peer_channels_disabled() {
        ...
        alice
            .set_config_bool(Config::WebndcRealtimeEnabled, false)
            .await
            .unwrap();

        // creates iroh endpoint as side effect
        send_webndc_realtime_advertisement(alice, MsgId::new(1))
            .await
            .unwrap();
        ...
        // creates iroh endpoint as side effect
        send_webndc_realtime_data(alice, MsgId::new(1), vec![])
            .await
            .unwrap();

        assert!(alice.ctx.iroh.read().await.is_none());

        leave_webndc_realtime(alice, MsgId::new(1)).await.unwrap();
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn test_leave_webndc_realtime_uninitialized() {
        let mut tcm = TestContextManager::new();
        let alice = &mut tcm.alice().await;

        alice
            .set_config_bool(Config::WebndcRealtimeEnabled, true)
            .await
            .unwrap();
