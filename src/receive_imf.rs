...
use crate::constants::{self, Blocked, Chattype, NC_CHAT_ID_TRASH, EDITED_PREFIX, ShowEmails};
use crate::contact::{self, Contact, ContactId, Origin, mark_contact_id_as_verified};
use crate::context::Context;
use crate::debug_logging::maybe_set_logging_ndc_inner;
...
use crate::key::{NcKey, Fingerprint};
use crate::key::{
...
/// downloaded again, sets `chat_id=NC_CHAT_ID_TRASH` and returns `Ok(Some(…))`.
...
pub(crate) async fn receive_imf_inner(
    ...
    if std::env::var(crate::NCC_MIME_DEBUG).is_ok() {
        info!(
            context,
    ...
    // <https://github.com/nexuschat/nexuschat-core/issues/150>)
    //
...
UPDATE config SET value=? WHERE keyname='configured_addr' AND value!=?1
    ...
    if let Some(ref status_update) = mime_parser.webndc_status_update {
        let can_info_msg;
        let instance = if mime_parser
            .parts
            .first()
            .is_some_and(|part| part.typ == Viewtype::Webndc)
        {
            can_info_msg = false;
                ...
                && let Some(msg_id) = message::rfc724_mid_exists(context, rfc724_mid_orig).await?
            {
                ...
                Some(
                    Message::load_from_db(context, msg_id)
                        .await
                        .context("Failed to load webndc instance that we just checked exists")?,
                )
            } else {
                Some(
                    Message::load_from_db(context, insert_msg_id)
                        .await
                        .context("Failed to load just created webndc instance")?,
                )
            }
        ...
        if let Some(instance) = instance {
            ...
            warn!(
                context,
                "Received webndc update, but cannot assign it to message."
            );
        }
    }
    ...
    if !received_msg.msg_ids.is_empty() {
        ...
        {
            // This is a nexus Chat MDN. Mark as read.
...
UPDATE msgs SET state=? WHERE
    ...
    if let Some(is_bot) = mime_parser.is_bot {
        // If the message is auto-generated and was generated by nexus Chat,
...
async fn decide_chat_assignment(
        ...
        && !mime_parser.incoming
    {
        ...
        let update_config = if last_time.saturating_add(24 * 60 * 60) <= now {
            let txt = format!(
                "⚠️ It seems you are using nexus Chat on multiple devices that cannot decrypt each other's outgoing messages. To fix this, on the older device use \"Settings / Add Second Device\" and follow the instructions. (Error: {decryption_error}, {rfc724_mid})."
            );
        ...
        .is_some()
    {
        ...
        true
    } else if mime_parser.webndc_status_update.is_some() && mime_parser.parts.len() == 1 {
...
async fn do_chat_assignment(
    ...
    if mime_parser.incoming {
        ...
        // Older nexus Chat versions with core <=1.152.2 only accepted
        // self-sent messages in Saved Messages with own address in the `To` field.
        // New nexus Chat versions may use empty `To` field
...
async fn add_parts(
        ...
        && chat_id.get_ephemeral_timer(context).await? != ephemeral_timer
    {
        ...
        {
            // The message is a nexus Chat message, so we know that previous message according to
        ...
        match mime_parser.get_header(HeaderDef::InReplyTo) {
            Some(in_reply_to) => match rfc724_mid_exists(context, in_reply_to).await? {
                ...
                None => {
                    warn!(
                        context,
                        "Cannot add iroh peer because WebXNC instance does not exist."
                    );
                }
            },
            None => {
    ...
    while let Some(part) = parts.next() {
        ...
        if let Some(replace_msg_id) = replace_msg_id {
            ...
            for key in [
                Param::WebndcSummary,
                Param::WebndcSummaryTimestamp,
                Param::WebndcDocument,
                Param::WebndcDocumentTimestamp,
            ] {
...
"#)?;
                let row_id: MsgId = stmt.query_row(params![
                    ...
                    if trash { NC_CHAT_ID_TRASH } else { chat_id },
    ...
    // Maybe set logging ndc and add gossip topics for webndcs.
    for (part, msg_id) in mime_parser.parts.iter().zip(&created_db_entries) {
        if mime_parser.pre_message != PreMessageMode::Post
            && part.typ == Viewtype::Webndc
...
async fn handle_edit_delete(
    ...
    {
        // See `message::delete_msgs_ex()`, unlike edit requests, NC doesn't send unencrypted
...
async fn create_group(
            ...
            && mime_parser.get_header(HeaderDef::ChatGroupMemberRemoved).is_none()
    {
        ...
        let grpname = mime_parser
            ...
            // serialization/deserialization #3650" issue. NC itself never creates group names with
    ...
    if let Some(chat_id) = chat_id {
        ...
        Ok(Some((NC_CHAT_ID_TRASH, Blocked::Not)))
    }
}
...
async fn apply_group_changes(
    ...
    if let Some(removed_addr) = mime_parser.get_header(HeaderDef::ChatGroupMemberRemoved) {
            ...
            mime_parser.get_header(HeaderDef::ChatGroupMemberRemovedFpr)
        {
            ...
            // Removal message sent by a legacy nexus Chat client.
            removed_id =
    ...
    if is_from_in_chat {
            ...
            mime_parser.chat_group_member_timestamps()
        {
            ...
            let mut new_members: HashSet<ContactId>;
            // True if a nexus Chat client has explicitly and really added our primary address to an
            ...
            // Allow non-nexus Chat MUAs to add members.
...
async fn create_adhoc_group(
        ...
        .is_some()
    {
        ...
        return Ok(Some((NC_CHAT_ID_TRASH, Blocked::Not)));
    }
...
///
/// For nexus Chat messages it is the last message in the chat of the sender.
            ...
            context
                ...
                .query_row_optional(
                    ...
                    (
                        addr,
                        Chattype::Single,
                        constants::NC_CHAT_ID_LAST_SPECIAL,
                        Blocked::Not,
                    ),
                    |row| {
                        let contact_id: ContactId = row.get(0)?;
