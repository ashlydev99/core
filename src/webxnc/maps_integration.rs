//! # Maps Webxnc Integration.
//!
//! A Maps Webxnc Integration uses `sendUpdate()` and `setUpdateListener()` as usual,
...
use crate::webxnc::{StatusUpdateItem, StatusUpdateItemAndSerial, StatusUpdateSerial};
use anyhow::Result;
...
mod tests {
    ...
    async fn test_maps_integration() -> Result<()> {
        ...
        let bytes = include_bytes!("../../test-data/webxnc/mapstest-integration-set.xnc");
        ...
        // Integrate Webxnc into a chat with Bob;
        ...
        let integration_id = t.init_webxnc_integration(Some(bob_chat_id)).await?.unwrap();
        assert!(!integration_id.is_special());

        let integration = Message::load_from_db(&t, integration_id).await?;
        let info = integration.get_webxnc_info(&t).await?;
        ...
        t.evtracker
            ...
            .await;
        let updates = t
            .get_webxnc_status_updates(integration_id, StatusUpdateSerial(0))
            .await?;
        ...
        // Integrate Webxnc into another group
        let group_id = create_group(&t, "foo").await?;
        let integration_id = t.init_webxnc_integration(Some(group_id)).await?.unwrap();
