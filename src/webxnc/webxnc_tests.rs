...
async fn test_is_webxnc_file() -> Result<()> {
    ...
    assert!(
        !t.is_webxnc_file(
            ...
            include_bytes!("../../test-data/webxnc/minimal.xnc")
        )
        .await?
    );
    assert!(
    ...
    assert!(
        !t.is_webxnc_file(
            "good-ext-no-index-html.xnc",
            include_bytes!("../../test-data/webxnc/no-index-html.xnc")
        )
        .await?
    );
    assert!(
        t.is_webxnc_file(
            "good-ext-good-zip.xnc",
            include_bytes!("../../test-data/webxnc/minimal.xnc")
        )
        .await?
    );
...
fn create_webxnc_instance(t: &TestContext, name: &str, bytes: &[u8]) -> Result<Message> {
    let mut instance = Message::new(Viewtype::File);
    instance.set_file_from_bytes(t, name, bytes, None)?;
    Ok(instance)
}

async fn send_webxnc_instance(t: &TestContext, chat_id: ChatId) -> Result<Message> {
    let mut instance = create_webxnc_instance(
        t,
        "minimal.xnc",
        include_bytes!("../../test-data/webxnc/minimal.xnc"),
    )?;
    let instance_msg_id = send_msg(t, chat_id, &mut instance).await?;
    assert_eq!(instance.viewtype, Viewtype::Webxnc);
    Message::load_from_db(t, instance_msg_id).await
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_send_webxnc_instance() -> Result<()> {
    let t = TestContext::new_alice().await;
    let chat_id = create_group(&t, "foo").await?;

    // send as .xnc file
    let instance = send_webxnc_instance(&t, chat_id).await?;
    assert_eq!(instance.viewtype, Viewtype::Webxnc);
    assert_eq!(instance.get_filename(), Some("minimal.xnc".to_string()));
    assert_eq!(instance.chat_id, chat_id);

    // sending using bad extension is not working, even when setting Viewtype to webxnc
    let mut instance = Message::new(Viewtype::Webxnc);
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_send_invalid_webxnc() -> Result<()> {
    let t = TestContext::new_alice().await;
    let chat_id = create_group(&t, "foo").await?;

    // sending invalid .xnc as file is possible, but must not result in Viewtype::Webxnc
    let mut instance = create_webxnc_instance(
        &t,
        "invalid-no-zip-but-7z.xnc",
        include_bytes!("../../test-data/webxnc/invalid-no-zip-but-7z.xnc"),
    )?;
    ...
    // sending invalid .xnc as Viewtype::Webxnc should fail already on sending
    let mut instance = Message::new(Viewtype::Webxnc);
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_set_draft_invalid_webxnc() -> Result<()> {
    let t = TestContext::new_alice().await;
    let chat_id = create_group(&t, "foo").await?;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_send_special_webxnc_format() -> Result<()> {
    let t = TestContext::new_alice().await;
    let chat_id = create_group(&t, "foo").await?;

    // chess.xnc is failing for some zip-versions, see #3476, if we know more details about why, we can have a nicer name for the test :)
    let mut instance = create_webxnc_instance(
        &t,
        "chess.xnc",
        include_bytes!("../../test-data/webxnc/chess.xnc"),
    )?;
    let instance_id = send_msg(&t, chat_id, &mut instance).await?;
    let instance = Message::load_from_db(&t, instance_id).await?;
    assert_eq!(instance.viewtype, Viewtype::Webxnc);

    let info = instance.get_webxnc_info(&t).await?;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_forward_webxnc_instance() -> Result<()> {
    let t = TestContext::new_alice().await;
    let chat_id = create_group(&t, "foo").await?;
    let instance = send_webxnc_instance(&t, chat_id).await?;
    t.send_webxnc_status_update(
        instance.id,
    ...
    assert_eq!(
        t.get_webxnc_status_updates(instance.id, StatusUpdateSerial(0))
            .await?,
    ...
    assert_eq!(
        t.get_webxnc_status_updates(instance2.id, StatusUpdateSerial(0))
            .await?,
        "[]"
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_resend_webxnc_instance_and_info() -> Result<()> {
    let mut tcm = TestContextManager::new();
    let alice = tcm.alice().await;
    let bob = tcm.bob().await;

    // Alice uses webxnc in a group
    alice.set_config_bool(Config::BccSelf, false).await?;
    let alice_grp = create_group(&alice, "grp").await?;
    let alice_instance = send_webxnc_instance(&alice, alice_grp).await?;
    assert_eq!(alice_grp.get_msg_cnt(&alice).await?, 2);
    alice
        .send_webxnc_status_update(
            alice_instance.id,
    ...
    // Alice adds Bob and resends already used webxnc
    add_contact_to_chat(
        &alice,
        alice_grp,
    ...
    // Bob receives webxnc, legacy info-messages updates are received and added to the chat.
    let bob_instance = bob.recv_msg(&sent1).await;
    bob.recv_msg_trash(&sent2).await;
    assert_eq!(bob_instance.viewtype, Viewtype::Webxnc);
    assert!(!bob_instance.is_info());
    assert_eq!(
        bob.get_webxnc_status_updates(bob_instance.id, StatusUpdateSerial(0))
            .await?,
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_receive_webxnc_instance() -> Result<()> {
    let t = TestContext::new_alice().await;
    receive_imf(
        &t,
        include_bytes!("../../test-data/message/webxnc_good_extension.eml"),
        false,
    )
    .await?;
    let instance = t.get_last_msg().await;
    assert_eq!(instance.viewtype, Viewtype::Webxnc);
    assert_eq!(instance.get_filename().unwrap(), "minimal.xnc");

    receive_imf(
        &t,
        include_bytes!("../../test-data/message/webxnc_bad_extension.eml"),
        false,
    )
    .await?;
    let instance = t.get_last_msg().await;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_webxnc_contact_request() -> Result<()> {
    let alice = TestContext::new_alice().await;
    let bob = TestContext::new_bob().await;

    // Alice sends an webxnc instance to Bob
    let alice_chat = alice.create_chat(&bob).await;
    let _alice_instance = send_webxnc_instance(&alice, alice_chat.id).await?;
    bob.recv_msg(&alice.pop_sent_msg().await).await;

    // Bob can start the webxnc from a contact request (get index.html)
    ...
    assert!(
        bob_instance
            .get_webxnc_blob(&bob, "index.html")
            .await
            .is_ok()
    );
    assert!(
        bob.send_webxnc_status_update(bob_instance.id, r#"{"payload":42}"#)
            .await
            .is_err()
    );
    assert_eq!(
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_delete_webxnc_instance() -> Result<()> {
    let t = TestContext::new_alice().await;
    let chat_id = create_group(&t, "foo").await?;
    let instance = send_webxnc_instance(&t, chat_id).await?;
    let now = tools::time();
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_delete_chat_with_webxnc() -> Result<()> {
    let t = TestContext::new_alice().await;
    let chat_id = create_group(&t, "foo").await?;
    let instance = send_webxnc_instance(&t, chat_id).await?;
    let now = tools::time();
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_delete_webxnc_draft() -> Result<()> {
    let t = TestContext::new_alice().await;
    let chat_id = create_group(&t, "foo").await?;
    ...
    t.send_webxnc_status_update(instance.id, r#"{"payload": 42}"#)
        .await?;
    assert_eq!(
        t.get_webxnc_status_updates(instance.id, StatusUpdateSerial(0))
            .await?,
...
async fn test_create_status_update_record() -> Result<()> {
    ...
    assert!(
        t.send_webxnc_status_update(instance.id, "\n\n\n")
            .await
            .is_err()
    );

    assert!(
        t.send_webxnc_status_update(instance.id, "bad json")
            .await
            .is_err()
    );
    ...
    let update_id2 = t
        ...
        .unwrap();
    assert_eq!(
        t.get_webxnc_status_updates(instance.id, update_id1).await?,
...
{"payload":true,"serial":4,"max_serial":4}]"#
    ...
    .await?;
    assert_eq!(
        t.get_webxnc_status_updates(instance.id, update_id2).await?,
...
async fn expect_status_update_event(t: &TestContext, instance_id: MsgId) -> Result<()> {
    let event = t
        .evtracker
        .get_matching(|evt| matches!(evt, EventType::WebxncStatusUpdate { .. }))
        .await;
    match event {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_send_webxnc_status_update() -> Result<()> {
    let alice = TestContext::new_alice().await;
    alice.set_config_bool(Config::BccSelf, true).await?;
    let bob = TestContext::new_bob().await;

    // Alice sends an webxnc instance and a status update
    let alice_chat = alice.create_email_chat(&bob).await;
    let alice_instance = send_webxnc_instance(&alice, alice_chat.id).await?;
    let sent1 = &alice.pop_sent_msg().await;
    assert_eq!(alice_instance.viewtype, Viewtype::Webxnc);
    assert!(!sent1.payload().contains("report-type=status-update"));

    alice
        .send_webxnc_status_update(alice_instance.id, r#"{"payload" : {"foo":"bar"}}"#)
        .await?;
    alice.flush_status_updates().await?;
    ...
    assert_eq!(
        alice
            .get_webxnc_status_updates(alice_instance.id, StatusUpdateSerial(0))
            .await?,
        r#"[{"payload":{"foo":"bar"},"serial":1,"max_serial":1}]"#
    );

    alice
        .send_webxnc_status_update(alice_instance.id, r#"{"payload":{"snipp":"snapp"}}"#)
        .await?;
    assert_eq!(
        alice
            .get_webxnc_status_updates(alice_instance.id, StatusUpdateSerial(0))
...
{"payload":{"snipp":"snapp"},"serial":2,"max_serial":2}]"#
    ...
    assert_eq!(alice2_instance.viewtype, Viewtype::Webxnc);
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_send_big_webxnc_status_update() -> Result<()> {
    let alice = TestContext::new_alice().await;
    alice.set_config_bool(Config::BccSelf, true).await?;
    let bob = TestContext::new_bob().await;
    ...
    let update1_str = r#"{"payload":{"foo":""#.to_string()
        ...
        + r#""}"#;
    alice
        .send_webxnc_status_update(alice_instance.id, &(update1_str.clone() + "}"))
        .await?;
    alice
        .send_webxnc_status_update(alice_instance.id, r#"{"payload" : {"foo":"bar2"}}"#)
        .await?;
    alice
        .send_webxnc_status_update(alice_instance.id, r#"{"payload" : {"foo":"bar3"}}"#)
        .await?;
    alice.flush_status_updates().await?;
    ...
    assert_eq!(
        bob.get_webxnc_status_updates(bob_instance.id, StatusUpdateSerial(1))
            .await?,
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_render_webxnc_status_update_object() -> Result<()> {
    let t = TestContext::new_alice().await;
    ...
    assert_eq!(
        t.render_webxnc_status_update_object(instance.id, first, last, None)
            .await?,
        (None, StatusUpdateSerial(u32::MAX))
    );

    t.send_webxnc_status_update(instance.id, r#"{"payload": 1}"#)
        .await?;
    let (object, first_new) = t
        .render_webxnc_status_update_object(instance.id, first, last, None)
        .await?;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_render_webxnc_status_update_object_range() -> Result<()> {
    let t = TestContext::new_alice().await;
    let chat_id = create_group(&t, "a chat").await?;
    let instance = send_webxnc_instance(&t, chat_id).await?;
    t.send_webxnc_status_update(instance.id, r#"{"payload": 1}"#)
        .await?;
    t.send_webxnc_status_update(instance.id, r#"{"payload": 2}"#)
        .await?;
    t.send_webxnc_status_update(instance.id, r#"{"payload": 3}"#)
        .await?;
    t.send_webxnc_status_update(instance.id, r#"{"payload": 4}"#)
        .await?;
    let (json, first_new) = t
        .render_webxnc_status_update_object(
            instance.id,
...
async fn test_pop_status_update() -> Result<()> {
    let t = TestContext::new_alice().await;
    let chat_id = create_group(&t, "a chat").await?;
    let instance1 = send_webxnc_instance(&t, chat_id).await?;
    let instance2 = send_webxnc_instance(&t, chat_id).await?;
    let instance3 = send_webxnc_instance(&t, chat_id).await?;
    assert!(t.smtp_status_update_get().await?.is_none());

    t.send_webxnc_status_update(instance1.id, r#"{"payload": "1a"}"#)
        .await?;
    t.send_webxnc_status_update(instance2.id, r#"{"payload": "2a"}"#)
        .await?;
    t.send_webxnc_status_update(instance2.id, r#"{"payload": "2b"}"#)
        .await?;
    t.send_webxnc_status_update(instance3.id, r#"{"payload": "3a"}"#)
        .await?;
    t.send_webxnc_status_update(instance3.id, r#"{"payload": "3b"}"#)
        .await?;
    t.send_webxnc_status_update(instance3.id, r#"{"payload": "3c"}"#)
        .await?;
    assert_eq!(
        t.sql
            .count("SELECT COUNT(*) FROM smtp_status_updates", ())
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_draft_and_send_webxnc_status_update() -> Result<()> {
    let alice = TestContext::new_alice().await;
    let bob = TestContext::new_bob().await;
    let alice_chat_id = alice.create_email_chat(&bob).await.id;

    // prepare webxnc instance,
    // status updates are not sent for drafts, therefore send_webxnc_status_update() returns Ok(None)
    let mut alice_instance = create_webxnc_instance(
        &alice,
        "minimal.xnc",
        include_bytes!("../../test-data/webxnc/minimal.xnc"),
    )?;
    ...
    expect_status_update_event(&alice, alice_instance.id).await?;
    alice
        .send_webxnc_status_update(alice_instance.id, r#"{"payload":42, "info":"i"}"#)
        .await?;
    expect_status_update_event(&alice, alice_instance.id).await?;
    assert_eq!(
        alice
    ...
    let bob_instance = bob.recv_msg(&sent1).await;
    assert_eq!(bob_instance.viewtype, Viewtype::Webxnc);
    assert_eq!(bob_instance.get_filename().unwrap(), "minimal.xnc");
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_send_webxnc_status_update_to_non_webxnc() -> Result<()> {
    let t = TestContext::new_alice().await;
    let chat_id = create_group(&t, "foo").await?;
    let msg_id = send_text_msg(&t, chat_id, "ho!".to_string()).await?;
    assert!(
        t.send_webxnc_status_update(msg_id, r#"{"foo":"bar"}"#)
            .await
            .is_err()
    );
    Ok(())
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_get_webxnc_blob() -> Result<()> {
    let t = TestContext::new_alice().await;
    let chat_id = create_group(&t, "foo").await?;
    let instance = send_webxnc_instance(&t, chat_id).await?;

    let buf = instance.get_webxnc_blob(&t, "index.html").await?;
    assert_eq!(buf.len(), 188);
    assert!(String::from_utf8_lossy(&buf).contains("document.write"));

    assert!(
        instance
            .get_webxnc_blob(&t, "not-existent.html")
            .await
            .is_err()
    );
    Ok(())
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_get_webxnc_blob_default_icon() -> Result<()> {
    let t = TestContext::new_alice().await;
    let chat_id = create_group(&t, "foo").await?;
    let instance = send_webxnc_instance(&t, chat_id).await?;

    let buf = instance.get_webxnc_blob(&t, WEBXNC_DEFAULT_ICON).await?;
    assert!(buf.len() > 100);
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_get_webxnc_blob_with_absolute_paths() -> Result<()> {
    let t = TestContext::new_alice().await;
    let chat_id = create_group(&t, "foo").await?;
    let instance = send_webxnc_instance(&t, chat_id).await?;
    ...
    assert!(instance.get_webxnc_blob(&t, "/not-there").await.is_err());
    Ok(())
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_get_webxnc_blob_with_subdirs() -> Result<()> {
    let t = TestContext::new_alice().await;
    let chat_id = create_group(&t, "foo").await?;
    let mut instance = create_webxnc_instance(
        &t,
        "some-files.xnc",
        include_bytes!("../../test-data/webxnc/some-files.xnc"),
    )?;
    chat_id.set_draft(&t, Some(&mut instance)).await?;
    ...
    let buf = instance.get_webxnc_blob(&t, "subdir/bla.txt").await?;
    assert_eq!(buf.len(), 4);
    assert!(String::from_utf8_lossy(&buf).starts_with("bla"));

    let buf = instance
        .get_webxnc_blob(&t, "subdir/subsubdir/text.md")
        .await?;
    assert_eq!(buf.len(), 24);
    assert!(String::from_utf8_lossy(&buf).starts_with("this is a markdown file"));

    let buf = instance
        .get_webxnc_blob(&t, "subdir/subsubdir/text2.md")
        .await?;
    assert_eq!(buf.len(), 22);
    assert!(String::from_utf8_lossy(&buf).starts_with("another markdown"));

    let buf = instance
        .get_webxnc_blob(&t, "anotherdir/anothersubsubdir/foo.txt")
        .await?;
    assert_eq!(buf.len(), 4);
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_parse_webxnc_manifest() -> Result<()> {
    let result = parse_webxnc_manifest(r#"key = syntax error"#.as_bytes());
    assert!(result.is_err());

    let manifest = parse_webxnc_manifest(r#"no_name = "no name, no icon""#.as_bytes())?;
    assert_eq!(manifest.name, None);

    let manifest = parse_webxnc_manifest(r#"name = "name, no icon""#.as_bytes())?;
    assert_eq!(manifest.name, Some("name, no icon".to_string()));

    let manifest = parse_webxnc_manifest(
        r#"name = "foo"
icon = "bar""#
            .as_bytes(),
    )?;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_parse_webxnc_manifest_min_api() -> Result<()> {
    let manifest = parse_webxnc_manifest(r#"min_api = 3"#.as_bytes())?;
    assert_eq!(manifest.min_api, Some(3));

    let result = parse_webxnc_manifest(r#"min_api = "1""#.as_bytes());
    assert!(result.is_err());

    let result = parse_webxnc_manifest(r#"min_api = 1.2"#.as_bytes());
    assert!(result.is_err());
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_parse_webxnc_manifest_source_code_url() -> Result<()> {
    let result = parse_webxnc_manifest(r#"source_code_url = 3"#.as_bytes());
    assert!(result.is_err());

    let manifest = parse_webxnc_manifest(r#"source_code_url = "https://foo.bar""#.as_bytes())?;
    assert_eq!(
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_webxnc_min_api_too_large() -> Result<()> {
    let t = TestContext::new_alice().await;
    let chat_id = create_group(&t, "chat").await?;
    let mut instance = create_webxnc_instance(
        &t,
        "with-min-api-1001.xnc",
        include_bytes!("../../test-data/webxnc/with-min-api-1001.xnc"),
    )?;
    send_msg(&t, chat_id, &mut instance).await?;

    let instance = t.get_last_msg().await;
    let html = instance.get_webxnc_blob(&t, "index.html").await?;
    assert!(String::from_utf8_lossy(&html).contains("requires a newer Nexus Chat version"));

    Ok(())
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_get_webxnc_info() -> Result<()> {
    let t = TestContext::new_alice().await;
    let chat_id = create_group(&t, "foo").await?;

    let instance = send_webxnc_instance(&t, chat_id).await?;
    let info = instance.get_webxnc_info(&t).await?;
    assert_eq!(info.name, "minimal.xnc");
    assert_eq!(info.icon, WEBXNC_DEFAULT_ICON.to_string());
    assert_eq!(info.send_update_interval, 1000);
    assert_eq!(info.send_update_max_size, RECOMMENDED_FILE_SIZE as usize);

    let mut instance = create_webxnc_instance(
        &t,
        "with-manifest-empty-name.xnc",
        include_bytes!("../../test-data/webxnc/with-manifest-empty-name.xnc"),
    )?;
    chat_id.set_draft(&t, Some(&mut instance)).await?;
    let info = instance.get_webxnc_info(&t).await?;
    assert_eq!(info.name, "with-manifest-empty-name.xnc");
    assert_eq!(info.icon, WEBXNC_DEFAULT_ICON.to_string());

    let mut instance = create_webxnc_instance(
        &t,
        "with-manifest-no-name.xnc",
        include_bytes!("../../test-data/webxnc/with-manifest-no-name.xnc"),
    )?;
    chat_id.set_draft(&t, Some(&mut instance)).await?;
    let info = instance.get_webxnc_info(&t).await?;
    assert_eq!(info.name, "with-manifest-no-name.xnc");
    assert_eq!(info.icon, WEBXNC_DEFAULT_ICON.to_string());

    let mut instance = create_webxnc_instance(
        &t,
        "with-minimal-manifest.xnc",
        include_bytes!("../../test-data/webxnc/with-minimal-manifest.xnc"),
    )?;
    chat_id.set_draft(&t, Some(&mut instance)).await?;
    let info = instance.get_webxnc_info(&t).await?;
    ...
    let mut instance = create_webxnc_instance(
        &t,
        "with-manifest-and-png-icon.xnc",
        include_bytes!("../../test-data/webxnc/with-manifest-and-png-icon.xnc"),
    )?;
    chat_id.set_draft(&t, Some(&mut instance)).await?;
    let info = instance.get_webxnc_info(&t).await?;
    ...
    let mut instance = create_webxnc_instance(
        &t,
        "with-png-icon.xnc",
        include_bytes!("../../test-data/webxnc/with-png-icon.xnc"),
    )?;
    chat_id.set_draft(&t, Some(&mut instance)).await?;
    let info = instance.get_webxnc_info(&t).await?;
    assert_eq!(info.name, "with-png-icon.xnc");
    assert_eq!(info.icon, "icon.png");

    let mut instance = create_webxnc_instance(
        &t,
        "with-jpg-icon.xnc",
        include_bytes!("../../test-data/webxnc/with-jpg-icon.xnc"),
    )?;
    chat_id.set_draft(&t, Some(&mut instance)).await?;
    let info = instance.get_webxnc_info(&t).await?;
    assert_eq!(info.name, "with-jpg-icon.xnc");
    assert_eq!(info.icon, "icon.jpg");

    let msg_id = send_text_msg(&t, chat_id, "foo".to_string()).await?;
    let msg = Message::load_from_db(&t, msg_id).await?;
    let result = msg.get_webxnc_info(&t).await;
    assert!(result.is_err());
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_get_webxnc_self_addr() -> Result<()> {
    let t = TestContext::new_alice().await;
    let chat_id = create_group(&t, "foo").await?;

    let instance = send_webxnc_instance(&t, chat_id).await?;
    let info1 = instance.get_webxnc_info(&t).await?;
    let instance = send_webxnc_instance(&t, chat_id).await?;
    let info2 = instance.get_webxnc_info(&t).await?;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_webxnc_info_summary() -> Result<()> {
    let alice = TestContext::new_alice().await;
    let bob = TestContext::new_bob().await;

    // Alice creates an webxnc instance and updates summary
    let alice_chat = alice.create_chat(&bob).await;
    let alice_instance = send_webxnc_instance(&alice, alice_chat.id).await?;
    let sent_instance = &alice.pop_sent_msg().await;
    let info = alice_instance.get_webxnc_info(&alice).await?;
    assert_eq!(info.summary, "".to_string());

    alice
        .send_webxnc_status_update(alice_instance.id, r#"{"summary":"sum: 1", "payload":1}"#)
        .await?;
    alice.flush_status_updates().await?;
    let sent_update1 = &alice.pop_sent_msg().await;
    let info = Message::load_from_db(&alice, alice_instance.id)
        .await?
        .get_webxnc_info(&alice)
        .await?;
    assert_eq!(info.summary, "sum: 1".to_string());

    alice
        .send_webxnc_status_update(alice_instance.id, r#"{"summary":"sum: 2", "payload":2}"#)
        .await?;
    alice.flush_status_updates().await?;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_webxnc_document_name() -> Result<()> {
    let alice = TestContext::new_alice().await;
    let bob = TestContext::new_bob().await;

    // Alice creates an webxnc instance and updates document name
    let alice_chat = alice.create_chat(&bob).await;
    let alice_instance = send_webxnc_instance(&alice, alice_chat.id).await?;
    let sent_instance = &alice.pop_sent_msg().await;
    let info = alice_instance.get_webxnc_info(&alice).await?;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_webxnc_info_msg() -> Result<()> {
    let alice = TestContext::new_alice().await;
    let bob = TestContext::new_bob().await;
    ...
    assert_eq!(info_msg.get_info_type(), SystemMessage::WebxncInfoMessage);
    ...
    assert_eq!(
        alice2
            .get_webxnc_status_updates(alice2_instance.id, StatusUpdateSerial(0))
            .await?,
        r#"[{"payload":"sth. else","info":"this appears in-chat","serial":1,"max_serial":1}]"#
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_webxnc_info_msg_cleanup_series() -> Result<()> {
    let alice = TestContext::new_alice().await;
    let bob = TestContext::new_bob().await;
    let alice_chat = alice.create_chat(&bob).await;
    let alice_instance = send_webxnc_instance(&alice, alice_chat.id).await?;
    ...
    alice
        .send_webxnc_status_update(alice_instance.id, r#"{"info":"i1", "payload":1}"#)
        .await?;
    alice.flush_status_updates().await?;
    let sent2 = &alice.pop_sent_msg().await;
    assert_eq!(alice_chat.id.get_msg_cnt(&alice).await?, E2EE_INFO_MSGS + 2);
    alice
        .send_webxnc_status_update(alice_instance.id, r#"{"info":"i2", "payload":2}"#)
        .await?;
    alice.flush_status_updates().await?;
    let sent3 = &alice.pop_sent_msg().await;
    assert_eq!(alice_chat.id.get_msg_cnt(&alice).await?, E2EE_INFO_MSGS + 2);
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_webxnc_info_msg_no_cleanup_on_interrupted_series() -> Result<()> {
    let t = TestContext::new_alice().await;
    let chat_id = create_group(&t, "c").await?;
    let instance = send_webxnc_instance(&t, chat_id).await?;

    t.send_webxnc_status_update(instance.id, r#"{"info":"i1", "payload":1}"#)
        .await?;
    ...
    t.send_webxnc_status_update(instance.id, r#"{"info":"i2", "payload":2}"#)
        .await?;
...
// check that `info.internet_access` is not set for normal, non-integrated webxnc -
// even if they use the deprecated option `request_internet_access` in manifest.toml
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_webxnc_no_internet_access() -> Result<()> {
    let t = TestContext::new_alice().await;
    ...
    for chat_id in [self_id, single_id, group_id, broadcast_id] {
        for internet_xnc in [true, false] {
            let mut instance = create_webxnc_instance(
                &t,
                "foo.xnc",
                if internet_xnc {
                    include_bytes!("../../test-data/webxnc/request-internet-access.xnc")
                } else {
                    include_bytes!("../../test-data/webxnc/minimal.xnc")
                },
            )?;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_webxnc_chatlist_summary() -> Result<()> {
    let t = TestContext::new_alice().await;
    let chat_id = create_group(&t, "chat").await?;
    let mut instance = create_webxnc_instance(
        &t,
    ...
    assert_eq!(summary.thumbnail_path.unwrap(), "webxnc-icon://last-msg-id");

    Ok(())
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_webxnc_and_text() -> Result<()> {
    let alice = TestContext::new_alice().await;
    let bob = TestContext::new_bob().await;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_webxnc_reject_updates_from_non_groupmembers() -> Result<()> {
    let mut tcm = TestContextManager::new();
    let alice = &tcm.alice().await;
    let bob = &tcm.bob().await;
    ...
    let instance = send_webxnc_instance(alice, chat_id).await?;
    bob.recv_msg(&alice.pop_sent_msg().await).await;
    let bob_instance = bob.get_last_msg().await;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_webxnc_delete_event() -> Result<()> {
    let alice = TestContext::new_alice().await;
    ...
    alice
        .evtracker
        .get_matching(|evt| matches!(evt, EventType::WebxncInstanceDeleted { .. }))
        .await;
    Ok(())
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn change_logging_webxnc() -> Result<()> {
    let alice = TestContext::new_alice().await;
    ...
    alice
        .evtracker
        .get_matching(|ev| matches!(*ev, EventType::WebxncStatusUpdate { .. }))
        .await;
    assert!(
        alice
            .sql
...
/// Tests extensibility of WebXNC updates.
///
/// If an update sent by WebXNC contains unknown properties,
...
///
/// This ensures new WebXNC can try to send new properties
/// added in later revisions of the WebXNC API
/// and this will not result in a failure to send the whole update.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_send_webxnc_status_update_extensibility() -> Result<()> {
    let alice = TestContext::new_alice().await;
    let bob = TestContext::new_bob().await;
    let alice_chat = alice.create_chat(&bob).await;
    let alice_instance = send_webxnc_instance(&alice, alice_chat.id).await?;
...
// webxnc instance and status updates.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_status_update_vs_delete_device_after() -> Result<()> {
    ...
    update.param.set_cmd(SystemMessage::WebxncStatusUpdate);
    update
        .param
...
async fn has_incoming_webxnc_event(
    t: &TestContext,
    ...
    t.evtracker
        .get_matching_opt(t, |evt| {
            if let EventType::IncomingWebxncNotify { msg_id, text, .. } = evt {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_webxnc_notify_one() -> Result<()> {
    let mut tcm = TestContextManager::new();
    let alice = tcm.alice().await;
    let bob = tcm.bob().await;
    ...
    let grp_id = alice
        ...
        .await;
    let alice_instance = send_webxnc_instance(&alice, grp_id).await?;
    let sent1 = alice.pop_sent_msg().await;
    let bob_instance = bob.recv_msg(&sent1).await;
    ...
    alice
        .send_webxnc_status_update(
            ...
            &format!(
                ...
                bob_instance.get_webxnc_self_addr(&bob).await?
            ),
        )
        .await?;
    alice.flush_status_updates().await?;
    ...
    assert!(!has_incoming_webxnc_event(&alice, info_msg, "").await);

    bob.recv_msg_trash(&sent2).await;
    let info_msg = bob.get_last_msg().await;
    assert!(info_msg.is_info());
    assert_eq!(info_msg.text, "Alice moved");
    assert!(has_incoming_webxnc_event(&bob, info_msg, "Your move!").await);

    fiona.recv_msg_trash(&sent2).await;
    let info_msg = fiona.get_last_msg().await;
    assert!(info_msg.is_info());
    assert_eq!(info_msg.text, "Alice moved");
    assert!(!has_incoming_webxnc_event(&fiona, info_msg, "").await);

    Ok(())
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_webxnc_notify_multiple() -> Result<()> {
    let mut tcm = TestContextManager::new();
    let alice = tcm.alice().await;
    let bob = tcm.bob().await;
    let fiona = tcm.fiona().await;
    ...
    alice
        .send_webxnc_status_update(
            ...
            &format!(
                ...
                bob_instance.get_webxnc_self_addr(&bob).await?,
                fiona_instance.get_webxnc_self_addr(&fiona).await?
            ),
    ...
    let info_msg = bob.get_last_msg().await;
    assert!(info_msg.is_info());
    assert!(has_incoming_webxnc_event(&bob, info_msg, "move, Bob").await);

    fiona.recv_msg_trash(&sent2).await;
    let info_msg = fiona.get_last_msg().await;
    assert!(info_msg.is_info());
    assert!(has_incoming_webxnc_event(&fiona, info_msg, "move, Fiona").await);

    Ok(())
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_webxnc_no_notify_self() -> Result<()> {
    let mut tcm = TestContextManager::new();
    let alice = tcm.alice().await;
    ...
    let alice2_instance = alice2.recv_msg(&sent1).await;
    assert_eq!(
        alice_instance.get_webxnc_self_addr(&alice).await?,
        alice2_instance.get_webxnc_self_addr(&alice2).await?
    );
    ...
    let info_msg = alice2.get_last_msg().await;
    assert!(info_msg.is_info());
    assert!(!has_incoming_webxnc_event(&alice2, info_msg, "").await);

    Ok(())
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_webxnc_notify_all() -> Result<()> {
    let mut tcm = TestContextManager::new();
    let alice = tcm.alice().await;
    let bob = tcm.bob().await;
    let fiona = tcm.fiona().await;
    ...
    let info_msg = bob.get_last_msg().await;
    assert_eq!(info_msg.text, "go");
    assert!(has_incoming_webxnc_event(&bob, info_msg, "notify all").await);

    fiona.recv_msg_trash(&sent2).await;
    let info_msg = fiona.get_last_msg().await;
    assert_eq!(info_msg.text, "go");
    assert!(has_incoming_webxnc_event(&fiona, info_msg, "notify all").await);

    Ok(())
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_webxnc_notify_muted() -> Result<()> {
    let mut tcm = TestContextManager::new();
    let alice = tcm.alice().await;
    let bob = tcm.bob().await;
    let fiona = tcm.fiona().await;
    ...
    assert!(!has_incoming_webxnc_event(&alice, info_msg, "notify all").await);
    ...
    let info_msg = bob.get_last_msg().await;
    assert_eq!(info_msg.text, "reply");
    assert!(has_incoming_webxnc_event(&bob, info_msg, "reply, Bob").await);

    fiona.recv_msg_trash(&sent3).await;
    let info_msg = fiona.get_last_msg().await;
    assert_eq!(info_msg.text, "reply");
    assert!(has_incoming_webxnc_event(&fiona, info_msg, "reply, Fiona").await);

    Ok(())
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_webxnc_notify_bob_and_all() -> Result<()> {
    let mut tcm = TestContextManager::new();
    let alice = tcm.alice().await;
    let bob = tcm.bob().await;
    let fiona = tcm.fiona().await;
    ...
    bob.recv_msg_trash(&sent2).await;
    fiona.recv_msg_trash(&sent2).await;
    assert!(has_incoming_webxnc_event(&bob, bob_instance, "notify bob").await);
    assert!(has_incoming_webxnc_event(&fiona, fiona_instance, "notify all").await);

    Ok(())
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_webxnc_notify_all_and_bob() -> Result<()> {
    let mut tcm = TestContextManager::new();
    let alice = tcm.alice().await;
    let bob = tcm.bob().await;
    let fiona = tcm.fiona().await;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_webxnc_href() -> Result<()> {
    let mut tcm = TestContextManager::new();
    let alice = tcm.alice().await;
    let bob = tcm.bob().await;

    let grp_id = alice.create_group_with_members("grp", &[&bob]).await;
    let instance = send_webxnc_instance(&alice, grp_id).await?;
    let sent1 = alice.pop_sent_msg().await;
    ...
    let info_msg = alice.get_last_msg().await;
    assert!(info_msg.is_info());
    assert_eq!(info_msg.get_webxnc_href(), Some("#foobar".to_string()));
