...
use crate::key::{self, NcKey, SignedSecretKey};
...
pub enum ImexMode {
    ...
    /// created by NC_IMEX_EXPORT_BACKUP and detected by imex_has_backup(). Importing a backup
...
pub async fn has_backup(_context: &Context, dir_name: &Path) -> Result<String> {
    ...
    while let Ok(Some(dirent)) = dir_iter.next_entry().await {
        ...
        if name.starts_with("nexus-chat")
            && name.ends_with(".tar")
...
fn get_next_backup_path(
    ...
    let stem = chrono::DateTime::<chrono::Utc>::from_timestamp(backup_time, 0)
        ...
        .format("nexus-chat-backup-%Y-%m-%d")
        .to_string();
...
mod tests {
    ...
    /// Tests importing a backup from Nexus Chat 1.30.3 for Android (core v1.86.0).
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
