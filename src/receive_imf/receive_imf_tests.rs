...
use crate::constants::NC_GCL_FOR_FORWARDING;
use crate::contact;
...
async fn test_adhoc_group_show_accepted_contact_accepted() {
    ...
    // accept Bob by accepting a nexus-message from Bob
    receive_imf(&t, MSGRMSG, false).await.unwrap();
    let chats = Chatlist::try_load(&t, 0, None, None).await.unwrap();
    assert_eq!(chats.len(), 1);
    ...
    // receive a non-nexus-message from Bob, shows up because of the show_emails setting
    receive_imf(&t, ONETOONE_NOREPLY_MAIL, false).await.unwrap();

    assert_eq!(chat::get_chat_msgs(&t, chat_id).await.unwrap().len(), 2);

    // let Bob create an adhoc-group by a non-nexus-message, shows up because of the show_emails setting
    receive_imf(&t, GRP_MAIL, false).await.unwrap();
    let chats = Chatlist::try_load(&t, 0, None, None).await.unwrap();
    ...
    receive_imf(
        ...
        format!(
            "Received: (Postfix, from userid 1000); Mon, 4 Dec 2006 14:51:39 +0100 (CET)\n\
             ...
             \n\
             Reporting-UA: Nexus Chat 1.28.0\n\
...
async fn test_escaped_from() {
    ...
    assert_eq!(msg.is_nc_message, MessengerMessage::Yes);
...
async fn test_escaped_recipients() {
    ...
    let msg = Message::load_from_db(&t, chats.get_msg_id(0).unwrap().unwrap())
        .await
        .unwrap();
    assert_eq!(msg.is_nc_message, MessengerMessage::No);
...
async fn test_concat_multiple_ndns() -> Result<()> {
    ...
    let raw = raw.replace(
        "Message-Id: <20200609184422.NCB6B1200DD@mout01.posteo.de>",
...
static GH_MAILINGLIST: &[u8] =
    ...
    To: nexuschat/nexuschat-core-rust <nexuschat-core-rust@noreply.github.com>\n\
    Subject: Let's put some [brackets here that] have nothing to do with the topic\n\
    Message-ID: <3333@example.org>\n\
    List-ID: nexuschat/nexuschat-core-rust <nexuschat-core-rust.nexuschat.github.com>\n\
...
static GH_MAILINGLIST2: &str = "Received: (Postfix, from userid 1000); Mon, 4 Dec 2006 14:51:39 +0100 (CET)\n\
    ...
    To: nexuschat/nexuschat-core-rust <nexuschat-core-rust@noreply.github.com>\n\
    Subject: [nexuschat/nexuschat-core-rust] PR run failed\n\
...
async fn test_github_mailing_list() -> Result<()> {
    ...
    assert_eq!(chat.name, "nexuschat/nexuschat-core-rust");
    ...
    let chats = Chatlist::try_load(&t.ctx, 0, None, None).await?;
    assert_eq!(chats.len(), 1);
    let chats = Chatlist::try_load(&t.ctx, NC_GCL_FOR_FORWARDING, None, None).await?;
    assert_eq!(chats.len(), 0);
...
static NC_MAILINGLIST: &[u8] = b"Received: (Postfix, from userid 1000); Mon, 4 Dec 2006 14:51:39 +0100 (CET)\n\
    From: Bob <bob@posteo.org>\n\
    To: nexus@codespeak.net\n\
    Subject: Re: [nexus-dev] What's up?\n\
    Message-ID: <38942@posteo.org>\n\
    List-ID: \"discussions about and around https://nexus.chat developments\" <nexus.codespeak.net>\n\
    List-Post: <mailto:nexus@codespeak.net>\n\
    Precedence: list\n\
    Date: Sun, 22 Mar 2020 22:37:57 +0000\n\
    \n\
    body\n";

static NC_MAILINGLIST2: &[u8] = b"Received: (Postfix, from userid 1000); Mon, 4 Dec 2006 14:51:39 +0100 (CET)\n\
    From: Charlie <charlie@posteo.org>\n\
    To: nexus@codespeak.net\n\
    Subject: Re: [nexus-dev] NC is nice!\n\
...
async fn test_classic_mailing_list() -> Result<()> {
    let t = TestContext::new_alice().await;
    receive_imf(&t.ctx, NC_MAILINGLIST, false).await.unwrap();
    let chats = Chatlist::try_load(&t.ctx, 0, None, None).await.unwrap();
    let chat_id = chats.get_chat_id(0).unwrap();
    chat_id.accept(&t).await.unwrap();
    let chat = Chat::load_from_db(&t.ctx, chat_id).await.unwrap();
    assert_eq!(chat.name, "nexus-dev");
    assert!(chat.can_send(&t).await?);
    assert_eq!(chat.get_mailinglist_addr(), Some("nexus@codespeak.net"));
    ...
    assert!(mime.contains("Subject: Re: [nexus-dev] What's up?\r\n"));
    ...
    assert!(mime.contains("To: <nexus@codespeak.net>\r\n"));
...
Hello mailinglist!\r\n"
    ...
    receive_imf(&t.ctx, NC_MAILINGLIST2, false).await?;
...
async fn test_other_device_writes_to_mailinglist() -> Result<()> {
    let t = TestContext::new_alice().await;
    receive_imf(&t, NC_MAILINGLIST, false).await.unwrap();
    ...
    let list_post_contact_id =
        Contact::lookup_id_by_addr(&t, "nexus@codespeak.net", Origin::Unknown)
            .await?
            .unwrap();
    ...
    assert_eq!(
        chat::get_chat_id_by_grpid(&t, "nexus.codespeak.net")
            .await?
            .unwrap(),
...
async fn test_block_mailing_list() {
    ...
    receive_imf(&t.ctx, NC_MAILINGLIST2, false).await.unwrap();
...
async fn test_mailing_list_multiple_names_in_subject() -> Result<()> {
    ...
    Message-ID: <3333@example.org>\n\
    List-ID: \"looong description of 'ola list', with foo, bar\" <nexus.codespeak.net>\n\
    Date: Sun, 22 Mar 2020 22:37:57 +0000\n\
    \n\
    hello\n",
        false,
...
async fn test_mailing_list_with_mimepart_footer() {
    ...
    // this footer should not become a text-message in nexus chat
...
async fn test_apply_mailinglist_changes_assigned_by_reply() {
    ...
    assert_eq!(
        ...
        "nexuschat-core-rust.nexuschat.github.com"
    )
}
...
async fn check_dont_show_in_contacts_list(addr: &str) {
    ...
    receive_imf(
        &t,
        format!(
            "Subject: Re: [nexuschat/nexuschat-core-rust] NC is the best repo on GitHub!
To: {addr}
References: <nexuschat/nexuschat-core-rust/pull/1625@github.com>
 <nexuschat/nexuschat-core-rust/pull/1625/c644661857@github.com>
From: alice@example.org
...
async fn check_alias_reply(from_nc: bool, chat_request: bool, group_request: bool) {
    let mut tcm = TestContextManager::new();
    let alice = tcm.alice().await;

    // Claire, a customer, sends a support request
    // to the alias address <support@example.org>.
    // If `chat_request` is true, Claire is using Nexus Chat,
    ...
    let claire_request = if group_request {
        format!(
            "Received: (Postfix, from userid 1000); Mon, 4 Dec 2006 14:51:39 +0100 (CET)\n\
             ...
             Message-ID: <non-nc-1@example.org>\n\
             {}\
    ...
    let msg_id = rfc724_mid_exists(&claire, "non-nc-1@example.org")
        .await
        .unwrap()
        .unwrap();
    ...
    let reply = if from_nc {
        ...
        "To: support@example.org, claire@example.org\n\
         From: bob@example.net\n\
         Subject: =?utf-8?q?Re=3A_i_have_a_question?=\n\
         References: <non-mc-1@example.org>\n\
         In-Reply-To: <non-nc-1@example.org>\n\
         Message-ID: <non-nc-2@example.net>\n\
         Date: Sun, 14 Mar 2021 16:04:57 +0000\n\
         Content-Type: text/plain\n\
         \n\
         hi claire, the version is 1.0, cheers bob"
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_alias_support_answer_from_nonnc_nonchat_nongroup() {
    check_alias_reply(false, false, false).await;
}
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_alias_support_answer_from_nonnc_nonchat_group() {
    check_alias_reply(false, false, true).await;
}
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_alias_support_answer_from_nonnc_chat_nongroup() {
    check_alias_reply(false, true, false).await;
}
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_alias_support_answer_from_nonnc_chat_group() {
    check_alias_reply(false, true, true).await;
}
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_alias_support_answer_from_nc_nonchat_nongroup() {
    check_alias_reply(true, false, false).await;
}
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_alias_support_answer_from_nc_nonchat_group() {
    check_alias_reply(true, false, true).await;
}
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_alias_support_answer_from_nc_chat_nongroup() {
    check_alias_reply(true, true, false).await;
}
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_alias_support_answer_from_nc_chat_group() {
...
async fn test_dont_show_all_outgoing_msgs_in_self_chat() {
    // Regression test for <https://github.com/nexuschat/nexuschat-android/issues/1940>:
...
=2D-
Sent with my Nexus Chat Messenger: https://nexus.chat
...
///
/// 1. If you blocked some spammer using NC, the 1:1 messages with that contact
...
async fn test_outgoing_undecryptable() -> Result<()> {
    ...
    assert!(
        dev_msg
            .text
            .starts_with("⚠️ It seems you are using Nexus Chat on multiple devices that cannot decrypt each other's outgoing messages. To fix this, on the older device use \"Settings / Add Second Device\" and follow the instructions. (Error:")
    );
...
/// Tests that NC takes the correct Message-ID from the encrypted message part, not the unencrypted
...
async fn test_adhoc_grp_name_no_prefix() -> Result<()> {
    ...
    let chat_id = receive_imf(
        ...
        b"Subject: Re: Once upon a time this was with the only Re: here\n\
          ...
          \n\
          Adding Alice the Nexus Chat lover",
        false,
    )
    .await?
    .unwrap()
...
async fn test_receive_vcard() -> Result<()> {
    ...
    async fn test(
        ...
        if vcard_contains_address {
            ...
            // VCards without an email address are not "nexuschat contacts",
        ...
        let parsed = nexuschat_contact_tools::parse_vcard(vcard);
