...
impl Config {
    ...
    async fn create_lock_task(_dir: PathBuf) -> Result<Option<JoinHandle<anyhow::Result<()>>>> {
        ...
        // iOS itself ensures that multiple instances of nexus Chat are not running.
        Ok(None)
    }
    ...
    async fn create_lock_task(dir: PathBuf) -> Result<Option<JoinHandle<anyhow::Result<()>>>> {
        ...
        if locked_rx.await.is_err() {
            bail!(
                "nexus Chat is already running. To use nexus Chat, you must first close the existing nexus Chat process, or restart your device. (accounts.lock file is already locked)"
            );
        };
        Ok(Some(lock_task))
    }
...
///
/// Even if nexus Chat itself does not hold the file lock,
