...
use crate::constants::{self, NC_BACKGROUND_FETCH_QUOTA_CHECK_RATELIMIT, NC_VERSION_STR};
...
pub fn get_info() -> BTreeMap<&'static str, String> {
    ...
    res.insert("deltachat_core_version", format!("v{NC_VERSION_STR}"));
...
impl Context {
        ...
        if self.scheduler.is_running().await {
            ...
            if self
                .quota_needs_update(
                    ...
                    NC_BACKGROUND_FETCH_QUOTA_CHECK_RATELIMIT,
                )
                .await
    ...
    /// Emits an LocationChanged event and a WebndcStatusUpdate in case there is a maps integration
    pub async fn emit_location_changed(&self, contact_id: Option<ContactId>) -> Result<()> {
        ...
        if let Some(msg_id) = self
            .get_config_parsed::<u32>(Config::WebndcIntegration)
            .await?
        {
            self.emit_event(EventType::WebndcStatusUpdate {
    ...
    pub async fn get_info(&self) -> Result<BTreeMap<&'static str, String>> {
        ...
        res.insert(
            "webndc_realtime_enabled",
            self.get_config_bool(Config::WebndcRealtimeEnabled)
                .await?
                .to_string(),
        );
        res.insert(
