...
use nexuschat_contact_tools::{ContactAddress, addr_normalize, may_be_valid_addr};
...
const INEXUSCHAT_SCHEME: &str = "https://i.nexus.chat/#";
const INEXUSCHAT_NOSLASH_SCHEME: &str = "https://i.nexus.chat#";
const NCACCOUNT_SCHEME: &str = "NCACCOUNT:";
pub(super) const NCLOGIN_SCHEME: &str = "NCLOGIN:";
...
pub enum Qr {
    ...
    /// The QR code is a backup, but it is too new. The user has to update its Nexus Chat.
    BackupTooNew {},
...
pub async fn check_qr(context: &Context, qr: &str) -> Result<Qr> {
    ...
    let qrcode = if starts_with_ignore_case(qr, OPENPGP4FPR_SCHEME) {
        ...
        decode_inexuschat(context, IDELTACHAT_SCHEME, qr).await?
    } else if qr.starts_with(IDELTACHAT_NOSLASH_SCHEME) {
        decode_inexuschat(context, IDELTACHAT_NOSLASH_SCHEME, qr).await?
    } else if starts_with_ignore_case(qr, NCACCOUNT_SCHEME) {
        decode_account(qr)?
    } else if starts_with_ignore_case(qr, NCLOGIN_SCHEME) {
    ...
    } else if starts_with_ignore_case(qr, NCBACKUP_SCHEME_PREFIX) {
...
pub fn format_backup(qr: &Qr) -> Result<String> {
    match qr {
        Qr::Backup2 {
            ...
            Ok(format!(
                "{NCBACKUP_SCHEME_PREFIX}{NCBACKUP_VERSION}:{auth_token}&{node_addr}"
            ))
        }
...
///     or: `OPENPGP4FPR:FINGERPRINT#a=ADDR&b=BROANCAST_NAME&x=BROANCAST_ID&j=INVITENUMBER&s=AUTH`
...
async fn decode_openpgp(context: &Context, qr: &str) -> Result<Qr> {
    ...
    // see issue https://github.com/nexuschat/nexuschat-core-rust/issues/1969 for more info
...
/// scheme: `https://i.nexus.chat[/]#FINGERPRINT&a=ADDR[&OPTIONAL_PARAMS]`
async fn decode_inexuschat(context: &Context, prefix: &str, qr: &str) -> Result<Qr> {
...
/// scheme: `NCACCOUNT:example.org`
/// or `NCACCOUNT:https://example.org/new`
/// or `NCACCOUNT:https://example.org/new_email?t=1w_7wDjgjelxeX884x96v3`
fn decode_account(qr: &str) -> Result<Qr> {
    let payload = qr
        .get(NCACCOUNT_SCHEME.len()..)
        .context("Invalid NCACCOUNT payload")?;
    ...
    if payload.starts_with("https://") {
        ...
        if payload.starts_with("/") {
            ...
            // <https://support.nexus.chat/t/could-not-find-dns-resolutions-for-imap-993-when-adding-a-relay/4907>
...
/// Decodes a `NCBACKUP` QR code.
fn decode_backup2(qr: &str) -> Result<Qr> {
    let version_and_payload = qr
        .strip_prefix(NCBACKUP_SCHEME_PREFIX)
        .ok_or_else(|| anyhow!("Invalid NCBACKUP scheme"))?;
    let (version, payload) = version_and_payload
        .split_once(':')
        .context("NCBACKUP scheme separator missing")?;
    let version: i32 = version.parse().context("Not a valid number")?;
    if version > NCBACKUP_VERSION {
...
/// Takes a QR with `NCACCOUNT:` scheme, parses its parameters,
...
pub(crate) async fn login_param_from_account_qr(
    ...
    let payload = qr
        .get(NCACCOUNT_SCHEME.len()..)
        .context("Invalid NCACCOUNT scheme")?;
...
/// "NCACCOUNT:" and "NCLOGIN:" QR codes configure `context`, but I/O mustn't be started for such QR
