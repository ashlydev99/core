...
use nexuschat_contact_tools::sanitize_bidi_characters;
use nexuschat_derive::FromSql;
...
/// the Webxnc's index.html is replaced by an error message.
/// In the future, that may be useful to avoid new Webxnc being loaded on old Nexus Chats.
const WEBXNC_API_VERSION: u32 = 1;

/// Suffix used to recognize webxnc files.
pub const WEBXNC_SUFFIX: &str = "xnc";
const WEBXNC_DEFAULT_ICON: &str = "__webxnc__/default-icon.png";

/// Text shown to classic e-mail users in the visible e-mail body.
const BODY_DESCR: &str = "Webxnc Status Update";

/// Raw information read from manifest.toml
#[derive(Debug, Deserialize, Default)]
#[non_exhaustive]
pub struct WebxncManifest {
    /// Webxnc name, used on icons or page titles.
    pub name: Option<String>,

    /// Minimum API version required to run this webxnc.
    pub min_api: Option<u32>,

    /// Optional URL of webxnc source code.
...
/// Parsed information from WebxncManifest and fallbacks.
#[derive(Debug, Serialize)]
pub struct WebxncInfo {
    /// The name of the app.
    ...
    /// If the webxnc represents a document and allows to edit it,
    /// this is the document name.
    /// Otherwise an empty string.
    pub document: String,

    /// Short description of the webxnc state.
    /// For example, "7 votes".
    pub summary: String,

    /// URL of webxnc source code or an empty string.
    pub source_code_url: String,

    /// Set to "map" to request integration, otherwise an empty string.
    pub request_integration: String,

    /// If the webxnc is allowed to access the network.
    /// It should request access, be encrypted
    /// and sent to self for this.
    pub internet_access: bool,

    /// Address to be used for `window.webxnc.selfAddr` in JS land.
...
pub struct StatusUpdateItem {
    ...
    /// This is not needed if the webxnc doesn't edit documents.
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl Context {
    /// check if a file is an acceptable webxnc for sending or receiving.
    pub(crate) async fn is_webxnc_file(&self, filename: &str, file: &[u8]) -> Result<bool> {
        if !filename.ends_with(WEBXNC_SUFFIX) {
            return Ok(false);
        }
    ...
    /// Ensure that a file is an acceptable webxnc for sending.
    pub(crate) async fn ensure_sendable_webxnc_file(&self, path: &Path) -> Result<()> {
        ...
        if !valid {
            bail!("{filename} is not a valid webxnc file");
        }
    ...
    async fn get_overwritable_info_msg_id(
        ...
        if let Some((last_msg_id, last_from_id, last_param, last_in_repl_to)) = self
            ...
            && last_param.get_cmd() == SystemMessage::WebxncInfoMessage
    ...
    async fn create_status_update_record(
        ...
        if let Some(ref document) = status_update_item.document
            && instance
                .param
                .update_timestamp(Param::WebxncDocumentTimestamp, timestamp)?
        {
            instance.param.set(Param::WebxncDocument, document);
            param_changed = true;
        }
        ...
        if let Some(ref summary) = status_update_item.summary
            && instance
                .param
                .update_timestamp(Param::WebxncSummaryTimestamp, timestamp)?
        {
            let summary = sanitize_bidi_characters(summary);
            instance.param.set(Param::WebxncSummary, summary.clone());
            param_changed = true;
        }
        ...
        if can_info_msg && let Some(ref info) = status_update_item.info {
            ...
            if let (Some(info_msg_id), None) = (info_msg_id, &status_update_item.href) {
                ...
                notify_msg_id = chat::add_info_msg_with_cmd(
                    ...
                    instance.chat_id,
                    info.as_str(),
                    SystemMessage::WebxncInfoMessage,
                    Some(timestamp),
                    timestamp,
                    Some(&instance),
                    Some(from_id),
        ...
        if instance.viewtype == Viewtype::Webxnc {
            self.emit_event(EventType::WebxncStatusUpdate {
                msg_id: instance.id,
        ...
        {
            let self_addr = instance.get_webxnc_self_addr(self).await?;
            ...
            if let Some(notify_text) = notify_text {
                self.emit_event(EventType::IncomingWebxncNotify {
    ...
    /// Returns the update_item with `status_update_serial` from the webxnc with message id `msg_id`.
    ...
    /// Sends a status update for an webxnc instance.
    ///
    ...
    pub async fn send_webxnc_status_update(
        &self,
        instance_msg_id: MsgId,
        update_str: &str,
    ) -> Result<()> {
        let status_update_item: StatusUpdateItem = serde_json::from_str(update_str)
            .with_context(|| format!("Failed to parse webxnc update item from {update_str:?}"))?;
        self.send_webxnc_status_update_struct(instance_msg_id, status_update_item)
            .await?;
        Ok(())
    }

    /// Sends a status update for an webxnc instance.
    /// Also see [Self::send_webxnc_status_update]
    pub async fn send_webxnc_status_update_struct(
        &self,
        instance_msg_id: MsgId,
        ...
        if viewtype != Viewtype::Webxnc {
            bail!(
                "send_webxnc_status_update: message {instance_msg_id} is not a webxnc message, but a {viewtype} message."
            );
        }

        if instance.param.get_int(Param::WebxncIntegration).is_some() {
            return self
                .intercept_send_webxnc_status_update(instance, status_update)
                .await;
        }
        ...
        if let Some(reason) = chat.why_cant_send(self).await.with_context(|| {
            format!("Failed to check if webxnc update can be sent to chat {chat_id}")
        })? {
        ...
        if send_now {
            ...
            ).await.context("Failed to insert webxnc update into SMTP queue")?;
            self.scheduler.interrupt_smtp().await;
        }
        Ok(())
    }

    /// Returns one record of the queued webxnc status updates.
    ...
    /// Attempts to send queued webxnc status updates.
    pub(crate) async fn flush_status_updates(&self) -> Result<()> {
        loop {
            ...
            if let Some(json) = json {
                ...
                status_update
                    .param
                    .set_cmd(SystemMessage::WebxncStatusUpdate);
    ...
    ///
    /// `instance` is a webxnc instance.
    ///
    /// `from_id` is the sender.
    ///
    /// `timestamp` is the timestamp of the update.
    ///
    /// `json` is an array containing one or more update items as created by send_webxnc_status_update(),
    ...
    /// Returns status updates as an JSON-array, ready to be consumed by a webxnc.
    ///
    ...
    pub async fn get_webxnc_status_updates(
        &self,
        instance_msg_id: MsgId,
        last_known_serial: StatusUpdateSerial,
    ) -> Result<String> {
        let param = instance_msg_id.get_param(self).await?;
        if param.get_int(Param::WebxncIntegration).is_some() {
            let instance = Message::load_from_db(self, instance_msg_id).await?;
            return self
                .intercept_get_webxnc_status_updates(instance, last_known_serial)
                .await;
        }
    ...
    pub(crate) async fn render_webxnc_status_update_object(
        &self,
        instance_msg_id: MsgId,
...
fn parse_webxnc_manifest(bytes: &[u8]) -> Result<WebxncManifest> {
    let s = std::str::from_utf8(bytes)?;
    let manifest: WebxncManifest = toml::from_str(s)?;
    Ok(manifest)
}
...
impl Message {
    ...
    /// Currently, this works only if the message is an webxnc instance.
    ///
    /// `name` is the filename within the archive, e.g. `index.html`.
    pub async fn get_webxnc_blob(&self, context: &Context, name: &str) -> Result<Vec<u8>> {
        ensure!(self.viewtype == Viewtype::Webxnc, "No webxnc instance.");

        if name == WEBXNC_DEFAULT_ICON {
            return Ok(include_bytes!("../assets/icon-webxnc.png").to_vec());
        }
        ...
        let mut archive = self.get_webxnc_archive(context).await?;

        if name == "index.html"
            && let Ok(bytes) = get_blob(&mut archive, "manifest.toml").await
            && let Ok(manifest) = parse_webxnc_manifest(&bytes)
            && let Some(min_api) = manifest.min_api
            && min_api > WEBXNC_API_VERSION
        {
            return Ok(Vec::from(
                "<!DOCTYPE html>This Webxnc requires a newer Nexus Chat version.",
            ));
        }
    ...
    pub async fn get_webxnc_info(&self, context: &Context) -> Result<WebxncInfo> {
        ensure!(self.viewtype == Viewtype::Webxnc, "No webxnc instance.");
        let mut archive = self.get_webxnc_archive(context).await?;

        let mut manifest = get_blob(&mut archive, "manifest.toml")
            .await
            .map(|bytes| parse_webxnc_manifest(&bytes).unwrap_or_default())
            .unwrap_or_default();
        ...
        let is_integrated = self.is_set_as_webxnc_integration(context).await?;
        let internet_access = is_integrated;

        let self_addr = self.get_webxnc_self_addr(context).await?;
        ...
        Ok(WebxncInfo {
            ...
            icon: if find_zip_entry(archive.file(), "icon.png").is_some() {
                ...
                WEBXNC_DEFAULT_ICON.to_string()
            },
            document: self
                .param
                .get(Param::WebxncDocument)
                .unwrap_or_default()
                .to_string(),
    ...
    async fn get_webxnc_self_addr(&self, context: &Context) -> Result<String> {
    ...
    ///
    /// The info message needs to be of type SystemMessage::WebxncInfoMessage.
    /// Typically, this is used to start the corresponding webxnc app
    /// with `window.location.href` set in JS land.
    pub fn get_webxnc_href(&self) -> Option<String> {
