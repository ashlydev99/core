...
#[cfg(test)]
pub use nexuschat_time::SystemTimeTools as SystemTime;
...
use crate::constants::{self, NC_ELLIPSIS, NC_OUTDATED_WARNING_DAYS};
...
pub(crate) fn truncate(buf: &str, approx_chars: usize) -> Cow<'_, str> {
    ...
    if count <= approx_chars + NC_ELLIPSIS.len() {
...
pub(crate) async fn truncate_msg_text(context: &Context, text: String) -> Result<(String, bool)> {
    ...
    Ok(truncate_by_lines(
        text,
        constants::NC_DESIRED_TEXT_LINES,
        constants::NC_DESIRED_TEXT_LINE_LEN,
    ))
}
...
async fn maybe_warn_on_outdated(context: &Context, now: i64, approx_compile_time: i64) {
    if now > approx_compile_time + NC_OUTDATED_WARNING_DAYS * 24 * 60 * 60 {
