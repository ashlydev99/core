...
impl PushSubscriber {
    ...
    pub(crate) async fn subscribe(&self, context: &Context) -> Result<()> {
        ...
        if http::post_string(
            context,
            "https://notifications.nexus.chat/register",
            format!("{{\"token\":\"{token}\"}}"),
        )
        .await?
        {
...
mod tests {
    ...
    fn test_encrypt_device_token() {
        let fcm_token = encrypt_device_token("fcm-nexus.delta:c67DVcpVQN2rJHiSszKNDW:APA91bErcJV2b8qG0IT4aiuCqw6Al0_SbydSuz3V0CHBR1X7Fp8YzyvlpxNZIOGYVDFKejZGE1YiGSaqxmkr9ds0DuALmZNDwqIhuZWGKKrs3r7DTSkQ9MQ").unwrap();
        let fcm_beta_token = encrypt_device_token("fcm-chat.nexus.beta:chu-GhZCTLyzq1XseJp3na:APA91bFlsfDawdszWTyOLbxBy7KeRCrYM-SBFqutebF5ix0EZKMuCFUT_Y7R7Ex_eTQG_LbOu3Ky_z5UlTMJtI7ufpIp5wEvsFmVzQcOo3YhrUpbiSVGIlk").unwrap();
