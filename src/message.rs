...
use nexuschat_contact_tools::{VcardContact, parse_vcard};
...
use crate::constants::{Blocked, Chattype, NC_CHAT_ID_TRASH, NC_MSG_ID_LAST_SPECIAL};
...
SELECT ?1, rfc724_mid, pre_rfc724_mid, timestamp, ?, ? FROM msgs WHERE id=?1
                ",
                (self, NC_CHAT_ID_TRASH, on_server),
            )
            .await?;
...
impl rusqlite::types::ToSql for MsgId {
    fn to_sql(&self) -> rusqlite::Result<rusqlite::types::ToSqlOutput<'_>> {
        if self.0 <= NC_MSG_ID_LAST_SPECIAL {
...
impl Message {
    ...
    pub(crate) async fn try_set_vcard(&mut self, context: &Context, path: &Path) -> Result<()> {
        ...
        if let Some(summary) = get_vcard_summary(&vcard) {
            ...
            warn!(context, "try_set_vcard: Not a valid NexusChat vCard.");
...
pub async fn delete_msgs_ex(
    ...
    if delete_for_all {
        ...
        if let Some(chat_id) = modified_chat_ids.iter().next() {
            ...
            // - These are usually chats with non-NC clients who won't respect deletion requests
            ...
            .count(
             ...
             WHERE m.id > ?
               ...
               AND EXISTS (SELECT * FROM imap WHERE rfc724_mid=m.rfc724_mid);",
                (NC_MSG_ID_LAST_SPECIAL, threshold_timestamp, self_chat_id),
            )
            .await?
    } else {
        context
...
/// Returns the 1st part of summary text (i.e. before the dash if any) for a valid NexusChat vCard.
pub(crate) fn get_vcard_summary(vcard: &[u8]) -> Option<String> {
    let vcard = str::from_utf8(vcard).ok()?;
    let contacts = nexuschat_contact_tools::parse_vcard(vcard);
    let [c] = &contacts[..] else {
        return None;
    };
    if !nexuschat_contact_tools::may_be_valid_addr(&c.addr) {
        return None;
    }
