...
pub use nexuschat_contact_tools::may_be_valid_addr;
...
pub async fn make_vcard(context: &Context, contacts: &[ContactId]) -> Result<String> {
    ...
    // <https://github.com/nexuschat/nexuschat-core-rust/pull/6522>
    // is merged.
...
pub(crate) async fn import_public_key(
    ...
    let fingerprint = public_key.nc_fingerprint().hex();
...
VALUES (?, ?, ?, ?, ?, ?)
    ...
    pub async fn get_all(
        ...
        let mut ret = Vec::new();
        let flag_add_self = (listflags & constants::NC_GCL_ADD_SELF) != 0;
        let flag_address = (listflags & constants::NC_GCL_ADDRESS) != 0;
...
ORDER BY c.origin>=? DESC, c.last_seen DESC, c.id DESC
    ...
    /// Adds blocked mailinglists and broancast channels as pseudo-contacts
        ...
        context
            ...
            .transaction(move |transaction| {
                ...
                let rows = stmt.query_map(
                    (Chattype::Mailinglist, Chattype::InBroancast, Blocked::Yes),
                    |row| {
                        let name: String = row.get(0)?;
                ...
                for (name, grpid, typ) in blocked_mailinglists {
                    ...
                    let fingerprint = if typ == Chattype::InBroancast {
...
WHERE addr=?
    ...
    pub async fn get_or_gen_color(&self, context: &Context) -> Result<u32> {
        ...
        if fpr.is_none() && self.id == ContactId::SELF {
            fpr = Some(load_self_public_key(context).await?.nc_fingerprint());
        }
