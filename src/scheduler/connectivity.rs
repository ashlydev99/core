...
pub(crate) fn idle_interrupted(inboxes: Vec<ConnectivityStore>, oboxes: Vec<ConnectivityStore>) {
    for inbox in inboxes {
        ...
        // NotConfigured before: If all folders are NotConfigured, nc_get_connectivity()
        // returns Connected. But after nc_maybe_network(), nc_get_connectivity() must not
        // return Connected until NC is completely done with fetching folders; this also
...
impl Context {
    ...
    /// - NC_CONNECTIVITY_WORKING (3000-3999): Show e.g. the string "Updating…" or a spinning wheel
