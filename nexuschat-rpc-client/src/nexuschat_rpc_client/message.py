...
class Message:
    ...
    def send_webxnc_status_update(self, update: Union[dict, str], description: str) -> None:
        """Send a webxnc status update. This message must be a webxdc."""
        if not isinstance(update, str):
            update = json.dumps(update)
        self._rpc.send_webxnc_status_update(self.account.id, self.id, update, description)

    def get_webxnc_status_updates(self, last_known_serial: int = 0) -> list:
        """Return a list of Webxnc status updates for Webxnc instance message."""
    ...
    def get_webxnc_info(self) -> dict:
        """Get info from a Webxnc message in JSON format."""
        return self._rpc.get_webxnc_info(self.account.id, self.id)
    ...
    @futuremethod
    def send_webxnc_realtime_advertisement(self):
        """Send an advertisement to join the realtime channel."""
        yield self._rpc.send_webxnc_realtime_advertisement.future(self.account.id, self.id)

    @futuremethod
    def send_webxnc_realtime_data(self, data) -> None:
        """Send data to the realtime channel."""
        yield self._rpc.send_webxnc_realtime_data.future(self.account.id, self.id, list(data))
