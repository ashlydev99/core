...
from . import Account, AttrDict, Bot, Chat, Client, NexusChat, EventType, Message
...
def remote_bob_loop(channel):
    ...
    from nexuschat_rpc_client import NexusChat, Rpc
    from nexuschat_rpc_client.pytestplugin import ACFactory
    ...
    with rpc:
        nc = NexusChat(rpc)
        channel.send(nc.rpc.get_system_info()["nexuschat_core_version"])
        acfactory = ACFactory(nc)
