...
def test_move_works(acfactory, direct_imap):
    ...
    ac2_direct_imap.create_folder("NexusChat")
...
def test_reactions_for_a_reordering_move(acfactory, direct_imap):
    """When a batch of messages is moved from Inbox to NexusChat folder with a single MOVE command,
    ...
    # order by NC, and most (if not all) mail servers provide only seconds precision.
    time.sleep(1.1)
    react_str = "\N{THUMBS UP SIGN}"
    msg1.send_reaction(react_str).wait_until_delivered()

    logging.info("moving messages to ac2's NexusChat folder in the reverse order")
    ac2_direct_imap = direct_imap(ac2)
    ac2_direct_imap.connect()
    for uid in sorted([m.uid for m in ac2_direct_imap.get_all_messages()], reverse=True):
        ac2_direct_imap.conn.move(uid, "NexusChat")
...
def test_move_works_on_self_sent(acfactory, direct_imap):
    ...
    ac1_direct_imap.create_folder("NexusChat")
...
def test_moved_markseen(acfactory, direct_imap):
    """Test that message already moved to NexusChat folder is marked as seen."""
    ac1, ac2 = acfactory.get_online_accounts(2)
    ac2_direct_imap = direct_imap(ac2)
    ac2_direct_imap.create_folder("NexusChat")
    ac2.set_config("mvbox_move", "1")
    ...
    # Emulate moving of the message to NexusChat folder by Sieve rule.
    ac2_direct_imap.conn.move(["*"], "NexusChat")
    ac2_direct_imap.select_folder("NexusChat")
...
def test_markseen_message_and_mdn(acfactory, direct_imap, mvbox_move):
    ...
    for ac in ac1, ac2:
        ...
        if mvbox_move:
            ...
            ac_direct_imap.create_folder("NexusChat")
    ...
    if mvbox_move:
        rex = re.compile("Marked messages [0-9]+ in folder NexusChat as seen.")
    else:
