...
from nexuschat_rpc_client import EventType, events
from nexuschat_rpc_client.const import DownloadState, MessageState
from nexuschat_rpc_client.pytestplugin import E2EE_INFO_MSGS
from nexuschat_rpc_client.rpc import JsonRpcError, Rpc


def test_system_info(rpc) -> None:
    system_info = rpc.get_system_info()
    assert "arch" in system_info
    assert "nexuschat_core_version" in system_info
...
def test_receive_imf_failure(acfactory) -> None:
    ...
    message = bob.get_message_by_id(msg_id)
    snapshot = message.get_snapshot()
    version = bob.get_info()["nexuschat_core_version"]
    assert (
        snapshot.text == "❌ Failed to receive a message:"
        " Condition failed: `!context.get_config_bool(Config::SimulateReceiveImfError).await?`."
        f" Core version {version}."
        " Please report this bug to nexus@merlinux.eu or https://support.nexus.chat/."
    )
...
def test_openrpc_command_line() -> None:
    """Test that "nexuschat-rpc-server --openrpc" command returns an OpenRPC specification."""
    out = subprocess.run(["nexuschat-rpc-server", "--openrpc"], capture_output=True, check=True).stdout
...
def test_early_failure(tmp_path) -> None:
    ...
    # A non-empty directory that is not a nexuschat accounts directory.
...
def test_configured_imap_certificate_checks(acfactory):
    ...
    # "cert_old_automatic" is the value old Nexus Chat core versions used
    # to mean user entered "imap_certificate_checks=0" (Automatic)
    # and configuration failed to use strict TLS checks
    # so it switched strict TLS checks off.
    #
    # New versions of Nexus Chat are not disabling TLS checks
